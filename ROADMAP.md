...
**Last updated:** 2026-09-23 (CLM-0319 — M3-04 customers admin routes: added 4 admin routes to customers service — GET /customers (list with q/status filters), GET /customers/:id (detail), POST /customers/:id/suspend, POST /customers/:id/reinstate. Added 3 new admin scopes (customers:admin:read/suspend/reinstate), suspension_reason_code column, listProfiles method, 13 new HTTP tests. 4 of 5 remaining M3-04 gaps closed. Prior: CLM-0318 — M3-04 drivers list route: added `GET /drivers` admin list route to drivers service with `drivers:admin:read` scope, `adminScoped` config (no beneficiary), `list(limit, offset)` repository method, 3 new HTTP tests, OpenAPI contract updated, authz-policy counts updated (137 ops, 114 scopes, 85 unclassified). 1 of 5 remaining M3-04 gaps closed. Prior: CLM-0317 — M3-04 orders prefix fix: removed `/api/orders` and `/api/audit` prefixes from admin portal stores; 3 of 9 M3-04 gaps closed; APP_API_ROUTES gap count 9→6.
