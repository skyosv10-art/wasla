...
**Last updated:** 2026-09-23 (CLM-0317 — M3-04 orders prefix fix: removed `/api/orders` and `/api/audit` prefixes from admin portal stores; 3 of 9 M3-04 gaps closed; APP_API_ROUTES gap count 9→6. Prior: CLM-0314 — M3-02 gap closure: `GET /orders/drivers/:driverPublicId/jobs` route added to orders service; 5 new HTTP tests; driver mini-app earnings store updated; OpenAPI contract updated; authz-policy test counts updated (136 ops, 113 scopes, 44 token-bound, 36 owner+token-bound); M3-02 route gap removed from APP_API_ROUTES.md. Prior: CLM-0313 — M3-08 retro gate audit: new governance check 24 (app→service route contract) measured 10 app calls with no service route; M3-02 and M3-04 downgraded to In Progress with gate files; M3-05 Completed (CI green on branch 38/38 and on main 36/36); main CI had produced no verdict since 2026-09-22T08:54Z because a stuck queued run held the concurrency group — cancelled, RISK-0052.)
