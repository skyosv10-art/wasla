...
**Last updated:** 2026-09-14 (M0-33 — the CI non-start reason is now read literally from the provider instead of being derived from the conclusion)
**Last milestone:** `scripts/ci-evidence.sh` stopped restating a conclusion as a reason. It had declared that GitHub "publishes no counterpart to `failure_reason`", so a job that never started was recorded with `reason = failure` — a word that reads as "the code failed" when nothing ran at all. That declared limit was **measured false**: `GET /check-runs/<job_id>/annotations` returns the cause verbatim, and for run `34792951183` on `main` it returns "The job was not started because recent account payments have failed or your spending limit needs to be increased". The extractor now downloads that annotation for every job with no steps, classifies it into a stable code (`account_billing_blocked`), and keeps the raw text beside it so the classification itself can be re-checked from the raw. An unmatched annotation stays `job_did_not_start`; `failure` is never written as a reason again. A second root fault was fixed with it: with no `GITHUB_TOKEN` the script issued an anonymous `curl`, and because this repository is **private** that returned `404` and was recorded as `api_unreachable_or_no_run` — i.e. a total CI outage was being hidden behind apparent unreachability. An authenticated `gh` transport is now the second path. The single rule is unchanged: what did not start is neither `PASS` nor `FAIL`. Six new fixture cases (22 passing, 0 failing) and two killed mutants (`annotation_blind`, `reason_is_conclusion`) measure this rather than assert it. The consequence is registered, not buried: **RISK-0039** — the pipeline has produced **zero verdicts since 2026-09-13** (0 successes in the last 100 runs), an account-level billing block no patch can clear.
