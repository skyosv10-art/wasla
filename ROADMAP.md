...
**Last updated:** 2026-09-24 (`CLM-0337` — Released CLM-0337. M2-08 Completed. Prior: `CLM-0336` — M2-08 Completed. Observability collector live on Render: wasla-observability service deployed and verified — /healthz, /api/v1/targets (14 services), /api/v1/alerts (4 SLI rules), /metrics, POST /v1/traces (OTLP), HTML dashboard all 200 OK. tsx devDependency fix (PR #446 merged). RISK-0053 closed. Prior: `CLM-0335` — M2-08 Stage B Node.js observability collector: `services/observability/` package created as Node.js replacement for 3 Docker-based services (Prometheus, Alertmanager, OTLP collector) that failed to build on Render Free. Scrapes /metrics from 14 services, evaluates SLI alert rules, receives OTLP traces, exposes dashboard. Added `render_web_service.wasla_observability` to Terraform. Added `tsx` to devDependencies for runtime. Prior: `CLM-0333` — M2-08 Stage B: observability stack deployed on Render — Prometheus (scrapes /metrics from 14 HTTP services), Alertmanager (SLI alert rules: availability ≥99%, latency p95 ≤500ms, error rate ≤5%), OTLP Collector (receives OpenTelemetry traces). `OTEL_EXPORTER_OTLP_ENDPOINT` set on 14 services. 3 Render free-plan services created. Terraform module at `infra/terraform/observability/`. Prior: `CLM-0330` — G8 scheduler made runnable: the CLM-0328 script never ran (TS syntax in `.mjs`, unexported import, tests that did not import it and were never run by CI, word-presence guard). Replaced by workspace package `@wasla/tick-scheduler` (15 real tests, 19-gate guard) and a new narrow production role `tick-scheduler` in the authorization matrix (no role held any tick scope before); Render cron jobs need a paid plan (`starter`, owner-approved); deployment pending merge. Prior: `CLM-0329` — M2-07 gate PASSED: all 14 items ✅ including G8 closed by CLM-0328. M2-07 and M2-09A moved to Ready for Gate. Prior: `CLM-0328` — G8 tick scheduler closed: standalone `scripts/tick-scheduler.mjs` + Render Cron Jobs Terraform `infra/terraform/cron/main.tf` + 14-gate governance check + 9 tests. M2-07 gate item 9 ⚠️→✅. `CLM-0327` Released — M2-09A discovery PR #433 merged green 38/38. Prior: `CLM-0326` Released — M3-07 Completed.
