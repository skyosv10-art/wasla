...
describe("api client network error mapping", () => {
  ...
  it("maps a fetch TypeError rejection to network_error (status 0)", async () => {
    ...
    await expect(apiClient.get("/customers/WS-1234567890/profile")).rejects.toMatchObject({
      status: 0,
  ...
  it("maps an aborted request to timeout (status 408)", async () => {
    ...
    const expectation = expect(apiClient.get("/customers/WS-1234567890/profile")).rejects.toMatchObject({
      status: 408,
      message: "timeout",
    });
