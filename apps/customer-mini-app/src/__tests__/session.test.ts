...
describe("Session Store", () => {
  ...
  it("stores token in memory only", () => {
    useSessionStore.getState().setSession("test-token", "WS-1234567890", Date.now() + 3600_000);
  ...
  it("reports expired session", () => {
    useSessionStore.getState().setSession("test-token", "WS-1234567890", Date.now() - 1000);
    expect(useSessionStore.getState().isAuthenticated()).toBe(false);
  });
});
