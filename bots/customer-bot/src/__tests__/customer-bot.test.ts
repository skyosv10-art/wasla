/**
 * Composition-root test of the customer bot.
 *
...
const MINI_APP_URL = "https://apps.wasla.test/customer";
...
describe("customer bot", () => {
  it("serves the customer bot", () => {
    expect(BOT).toBe("customer");
    expect(BOT_MINI_APP[BOT]).toBe("customer");
  });

  it("exposes its own Mini App from the environment", async () => {
    const { app } = build();

    const response = await app.inject({ method: "GET", url: "/channel/customer/mini-app", headers: signFor("GET", "/channel/customer/mini-app") });

    expect(response.statusCode).toBe(200);
    expect(response.json()).toMatchObject({ bot: "customer", mini_app: "customer", url: MINI_APP_URL });
    await app.close();
  });

  it.each(["driver", "partner"])("refuses to serve the %s bot", async (other) => {
    const { app } = build();
