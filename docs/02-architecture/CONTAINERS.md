...
>
> **Last Updated:** 2026-08-23 · **Status:** Baseline v1.0 (+ خدمة Driver Subscription & Referral §4.7 — Phase 10 · MR 1/6: العقود الكنسية وحزمة الأنواع موثّقة، ولا كودَ خدمةٍ بعد) (+ خدمة Customer Core §4.1 — Phase 04: العقود والأنواع المُكتبة مُنفَّذة) (+ خدمة Order Engine §4.2 — Phase 06: العقود والأنواع المُكتبة وجدول الانتقالات موثّقة) (+ خدمتا Matching وDispatch §4.3 — Phase 07: العقود والأنواع المُكتبة موثّقة) (+ طبقة القنوات §5.1 — Phase 03: نواة channel-core ومُهيّئ telegram-adapter وطبقة تشغيل البوتات `bot-runtime` مع البوتات الثلاثة ومُهيّئات `channel-postgres` ودعم المجموعات مُنفَّذة) · **Related Team:** Team 09 (Data) · Team 10 (DevOps) · Team 12 (Integration)
...
**وما لا يوجد في هذه الخدمة بقرار** (ADR-014 القرار 7 · 9): لا إيقاف ولا حجب ولا عقوبة · ولا اشتراكات ولا مكافآت · ولا تسعيرٌ يعتمد النقاط · ولا تعليقٌ نصّي ولا بلاغٌ ولا تنقيح · ولا بيانات شخصية ولا مُعرّف قناة — لا عموداً ولا مساراً ولو معطّلاً. مالكوها: Phase 15 (القرار الإداريّ) · Phase 10 (الاشتراكات) · Phase 16 (البلاغ والتنقيح) · **Phase 17 (الفوترة — Billing & Store Fees)**.

> **تصحيحٌ موثّق (Phase 10 · MR 1/6):** كان هذا السطر يقول «Phase 19 (الدفع)»، و`Phase 19` في [ROADMAP](../16-progress/ROADMAP.md) هو **Security Hardening** لا الدفع؛ ومالكُ المال هو **Phase 17**. صُحّح هنا لأنّ Phase 10 يتعاقد على هذا الحدّ بعينه: هذه الخدمة تنقل `payment_reference` مُعتِماً ولا ترى مبلغاً ([ADR-015](../15-decisions/ADR-015-driver-subscription-entitlement-ledger-and-derived-referral-rewards.md) القرار 6)، فإحالةُ القارئ إلى طورٍ خاطئ تجعله يبحث عن مالكٍ لا وجود له.
...
### 4.7 خدمة Driver Subscription & Referral — Phase 10

خدمة **موجودة في الشجرة الأصلية** أعلاه (`subscriptions`)، و[ADR-015](../15-decisions/ADR-015-driver-subscription-entitlement-ledger-and-derived-referral-rewards.md) يُثبّت حدودها. القاعدة الحاكمة: **الاشتراك دفترُ مُدَدٍ والحالةُ مُشتقّةٌ منه، والمكافأةُ مُدّةٌ لا عدّاد، والخدمة لا ترى مالاً ولا تعاقب أحداً**.

| الخدمة | المسار | الغرض | الحالة |
|---|---|---|---|
| subscriptions | `services/subscriptions/` | كتالوجُ الخطط المجمَّدة وامتيازاتها · دفترُ المُدد append-only · الحالةُ الأربع المُشتقّة (`trial · active · expired · community`) وانتقالاتُها المُسلسَلة · رموزُ الإحالة ومطالباتُها · التأهيلُ المُشتَقّ من وقائع السمعة · مكافأةٌ واحدةٌ لكل إحالةٍ تُطبَّق كمُدّة · النبضة | **عقود كنسية (1/6) · Phase 10** — [العقود](../../services/subscriptions/contracts/README.md) · [نموذج المجال](../03-domain/DRIVER_SUBSCRIPTION_REFERRAL.md) — **والباقي: طبقةُ مجالٍ نقيّة (2/6) · استمراريّة Postgres (3/6) · طبقة HTTP على 8093 (4/6) · مستهلكُ حقائق السمعة وناشرُ الصادر (5/6) · بوابةُ الخروج (6/6)** |

**انحرافٌ موثّق في العدد لا في الموضع:** `services/referrals/` تبقى **فارغة بقرار** ([ADR-015](../15-decisions/ADR-015-driver-subscription-entitlement-ledger-and-derived-referral-rewards.md) القرار 1) — مكافأةُ الإحالة **مُدّةٌ في دفتر الاشتراك نفسه**، وخدمةٌ ثانية تعني كتابةً في دفترِ غيرِها أو نسخةً ثانيةً من الحالة تتباعد بصمت. والسابقةُ قائمة: `services/fraud/` بقيت فارغةً للسبب نفسه في الطور 09 (ADR-014 القرار 1). والانحراف مُعلَنٌ في `README` العقود وفي حارس اختبار، لا مكتوماً في الكود.

المنفذ: **subscriptions = 8093** (بعد `reputation` = 8092)، وهو ثابت مُصدّر من حزمة العقد (`SUBSCRIPTION_SERVICE_PORT`) لا رقماً منسوخاً باليد — درس MR 5b/6 من الطور 07، وحارسُ `boundary.test.ts` يمنع انتزاع منفذٍ خصّصه طورٌ سابق. و**لم يُستمَع عليه بعد**: طبقةُ HTTP في المراجعة 4/6، وهذا دَينٌ مُعلَنٌ لا إنجازٌ مُدَّعى.

اتجاه الاعتماد أحادي وملزم، ولا يُعكس:

```text
reputation    → subscriptions   (حقائقُ المُحال تصل عدداً: qualifying_fact_count)
subscriptions ↛ reputation      (لا FK إلى دفتر الوقائع ولا استعلامَ عبر الحدّ)
subscriptions ↛ drivers         (لا إيقافَ ولا تعليقَ أهليّة: تلك ملكُ services/drivers — ADR-012 القرار 3)
subscriptions ↛ orders          (لا قراءةَ طلبٍ ولا حكمَ على أهليّة قبوله: الاستحقاقُ يُقرأ ويُطبّقه غيرُها)
subscriptions ↛ billing         (لا مبلغَ ولا فاتورة: Phase 17 يملك المال — ADR-015 القرار 6)
```

**ما لا تعرفه خدمةُ الاشتراك:** المبلغَ والعملةَ والفاتورةَ ووسيلةَ الدفع، وهويّةَ السائق واسمَه وهاتفَه، وآلةَ حالة الطلب، وقناةَ التوصيل، وتفاصيلَ وقائع السمعة (تعرف عددَها لا صفوفَها). **وما لا تملكه:** إيقافَ أحدٍ أو حجبَه — تُعلن استحقاقاً ويبني عليه غيرُها في خدمته ويملك أثره.

**تسعةُ حدود يفرضها حارس اختبار لا مراجعة بشرية** (أربعةُ حرّاس تقرأ العقود **من القرص** لا نسخةً منها في الكود):

- **المُدّة تُخزَّن والحالةُ تُشتقّ**: `POST /subscriptions/{driverPublicId}/recompute` يُعيد بناء الحالة من الدفتر، وحذفُ عمود الحالة عملٌ بلا خسارةٍ في المعنى.
- **لا مالَ في العقد كلّه**: لا `NUMERIC` ولا `MONEY` ولا عمودَ مبلغٍ أو عملةٍ أو فاتورة، ولا حقلَ مالٍ في أيّ حمولة حدث، ولا رمزَ خطأٍ يقول «فشل الدفع» — `payment_reference` مُعتِمٌ يُنقَل ولا يُفسَّر.
- **التجديدُ ليس انتقالاً**: لا `active → active` في جدول الانتقالات المُعلَن؛ مُدّةٌ تُضاف إلى الدفتر ولا سطرَ انتقالٍ معها.
- **الخدمة لا تعاقب**: `community` أرضيّةُ استحقاقٍ (`accept_orders` باقٍ بسقفٍ يوميّ) لا حجب؛ ولا عمودَ `is_suspended` ولا مسارَ إيقافٍ ولا رمزَ خطأٍ عقابيّ ولا حدثَ `subscription.driver_suspended`.
- **الأرقام بياناتٌ لا كود**: التجربةُ والمُدّةُ والمهلةُ والمكافأةُ والعتبةُ والنافذةُ في **نسخةِ خطّةٍ مجمَّدة** (`saudi-driver-monthly` v1)، وكلُّ صفٍّ مُشتَقٍّ يحمل `plan_version`.
- **الزمن نبضةٌ لا مؤقّت**: `POST /subscriptions/tick` وحده يُحقّق الانقضاءَ ونهايةَ المهلة؛ فإعادةُ تشغيل الخدمة لا تُفقِد انقضاءً ولا تُنتج اثنين.
- **التأهيلُ مُشتَقٌّ من وقائعَ مُسجَّلة**: لا مكافأةَ على تسجيلٍ مجرّد؛ والعددُ والعتبةُ يعبران في الحدث كي يكون «تأهّل» قابلاً للمراجعة.
- **مكافأةٌ واحدةٌ لكل إحالة**: `ux_referral_rewards_referral` قيدٌ في القاعدة لا فحصٌ في الكود، لأنّ التسليم at-least-once.
- **الرفضُ حالةٌ لا حدث**: `rejected` بسببٍ من قائمةٍ مغلقة تُقرأ بمُرشِّح؛ ونشرُ رفضٍ يدعو مستهلكاً إلى بناء عقوبةٍ لا مالكَ لها.

**وما لا يوجد في هذه الخدمة بقرار** (ADR-015 القرار 4 · 6): لا إيقافَ ولا حجب · ولا مبلغَ ولا فاتورةَ ولا استردادَ ولا وسيلةَ دفع · ولا تسعيرٌ يعتمد السمعة · ولا بياناتٍ شخصيةٍ ولا مُعرّفَ قناة — لا عموداً ولا مساراً ولو معطّلاً. مالكوها: `services/drivers` (الإيقاف) · Phase 15 (القرار الإداريّ) · **Phase 17 (الفوترة)**.

**الدَين المُعلَن:** لا كودَ خدمةٍ بعد (المراجعات 2/6 → 6/6) · زرعُ خطّة الإطلاق في المراجعة 2/6 · ناشرُ `subscription_outbox` (نفس دَين الأطوار 06 · 07 · 08 · 09، واختيارُ الناقل في Phase 11) · لا قياسَ على Postgres حقيقيّ (لا محرّك في بيئة العمل).

الأنواع المُكتبة في `packages/contracts/subscription/` (`@wasla/contracts-subscription`).
...
| الحزمة | المسار | الغرض |
|---|---|---|
| contracts | `packages/contracts/` | API/Event/Data/Error contracts (Contract First) — حزمة لكل مجال: `identity` · `geography` · `channel` (§5.1) · `customer` (§4.1) · `order` (§4.2) · `matching` و`dispatch` (§4.3) · `driver` (§4.4) · `negotiation` (§4.5) · `reputation` (§4.6) · `subscription` (§4.7) |
