...
- **ولا مسارَ إعادةِ معالجةٍ لصفٍّ مسمومٍ** — **رُفِعَ في المراجعةِ 22/N** (§1أأ).

---

## 1أأ. ما أضافتْهُ المراجعةُ 22/N (مرجعٌ)

ترفعُ هذه المراجعةُ **الدَّينَ الثانيَ المُعلَنَ في [§4.23](../15-decisions/ADR-026-store-orders-and-delivery-boundary.md)**
(«لا مسارَ إعادةٍ لصفٍّ `poisoned`») — والتفصيلُ في
[§4.24](../15-decisions/ADR-026-store-orders-and-delivery-boundary.md):

- **مسارٌ رابعَ عشرَ `POST /delivery/relay/dead-letters/{ledger}/{eventId}/requeue`**
  (§2.3د) يُعيدُ صفّاً **مسموماً واحداً** إلى الطابورِ — **بصلاحيّةٍ ثانيةَ عشرةَ**
  `delivery:ops:relay-dead-letters:requeue` **مفصولةٍ عن `…:read`**. وضمُّها إلى
  القراءةِ كانَ يجعلُ كلَّ رمزِ لوحةِ رصدٍ مسروقٍ قادراً على إرجاعِ المُرحِّلَينِ
  إلى الصفرِ مِراراً.
- **حركتانِ في معاملةٍ واحدةٍ:** رفعُ نهائيّةِ الحكمِ (`poisoned` → `pending`)
  **ثمَّ** إرجاعُ نقطةِ تقدُّمِ الدفترِ إلى الصفرِ. والأولى وحدَها كانت تُسقِطُ
  الصفَّ من **مقياسِ §4.23** بلا أن يُقرأَ — **فقدٌ أخفى ممّا كانَ**.
- **`202` لا `200`، و`requeued` لا `reprocessed`:** النداءُ قَبِلَ الإعادةَ ولم
  يُنجِزْها؛ والصفُّ قد يُسَمُّ ثانيةً فوراً إن كانَ سببُهُ ثابتاً.
- **والدليلُ لا يُمحى:** `attempt_count` و`last_error` **يبقيانِ** — هُما الأثرُ
  الوحيدُ على لِمَ سُمَّ الصفُّ، والدفترُ لا يحفظُ تاريخاً.
- **وكلفةُ الإرجاعِ منشورةٌ في الجسمِ** (`rewind_cost: "full_rescan_from_zero"`)
  لا مخفيّةٌ خلفَ نجاحٍ.
- **ورفضانِ مُفرَّقانِ:** `404` «لا صفَّ» و`409` «ليسَ مسموماً» **بالحالةِ
  المقروءةِ في الرسالةِ**.
- **`gates_readiness: false` منشورٌ**: هذا المسارُ لا يمسُّ `GET /delivery/ready`.
...
- **ومسارُ إعادةِ المعالجةِ صارَ قائماً** في المراجعةِ 22/N (§2.3د) — ولا
  يُبدِّلُ هذا المقياسَ: هذا يقرأُ، وذاكَ يكتبُ، وبصمةُ هذا لا تتغيّرُ.

### 2.3د إعادةُ المسمومِ: حركتانِ معاً، وكلفةٌ مُعلَنةٌ (22/N)

```json
POST /delivery/relay/dead-letters/dispatch/11111111-2222-4333-8444-555555555555/requeue
...
- **`{ledger}` من قائمةٍ مُصرَّحةٍ حصراً** (`dispatch` · `marketplace_inventory`)
  و`{eventId}` UUID — وما دونَهُما **400 ولا يُنادى المنفذُ أصلاً**. وأسماءُ
  الجداولِ تُركَّبُ في نصِّ الاستعلامِ (Postgres لا يُمعامِلُها)، وأمانُ ذلكَ
  **ليسَ في التهذيبِ** بل في أنَّ **لا مُدخَلَ يبلغُ النصَّ**.
- **حركتانِ في معاملةٍ واحدةٍ:** `poisoned` → `pending` **ثمَّ** إرجاعُ نقطةِ
  التقدُّمِ إلى الصفرِ، على اتّصالٍ واحدٍ بـ`FOR UPDATE` على الصفِّ. والأولى
  وحدَها كانت تُخرِجُ الصفَّ من **مقياسِ §2.3ج** بلا أن يُقرأَ — فقدٌ **أخفى**.
- **`202` لا `200`:** النداءُ **قَبِلَ** الإعادةَ. المُرحِّلُ يقرأُ الصفَّ في
  دورةٍ لاحقةٍ، وقد يُسَمُّ ثانيةً فوراً إن كانَ سببُهُ ثابتاً — وجوابٌ يقولُ
  «عولِجَ» كانَ يُغلِقُ حادثةً على فقدٍ قائمٍ.
- **`attempt_count` و`last_error` لا يُمَسّانِ.** هُما الأثرُ الوحيدُ على لِمَ
  سُمَّ الصفُّ. **والأثرُ المُترتِّبُ مُعلَنٌ:** المُرحِّلُ يحسبُ
  `attempt = attempt_count + 1`، فصفٌّ استنفدَ محاولاتِهِ ينالُ **محاولةً واحدةً
  فقط** ثمَّ يُسَمُّ ثانيةً بسببٍ جديدٍ.
- **`rewind_cost` منشورٌ:** الإرجاعُ إلى الصفرِ لأنَّ الدفترَ لا يحفظُ
  `occurred_at`، والتماثُليّةُ تجعلُ كلَّ صفٍّ نهائيٍّ في الطريقِ **لا عملاً**.
  الثمنُ مسحٌ من أوّلِ الصندوقِ الصادرِ — **قراءةٌ لا كتابةٌ**.
- **وتحديثٌ بالإضافةِ (23/N · [ADR-026 §4.25](../15-decisions/ADR-026-store-orders-and-delivery-boundary.md) · M5-13R · CLM-0174):** الإعادةُ
  الآنَ تُحيزُ **قفلَ مُستهلِكِ الدفترِ** (`pg_advisory_xact_lock` بمفتاحِ
  `hashtext(consumerId)`) **قبلَ أوّلِ قراءةٍ** في معاملتِها، والمُرحِّلُ يمتلكُ
  القفلَ نفسَهُ على امتدادِ دفعتِهِ. **فالنداءُ قد ينتظرُ دفعةً جاريةً** قبلَ أن
  يلتزمَ — انتظارٌ **محدودٌ بالبناءِ** بامتصاصِ دفعةٍ واحدةٍ (`batchSize` صفّاً)،
  لا مهلةً في النداءِ ولا في الخادمِ، وطابورُ القفلِ FIFO فلا تجويعَ. وهذا
  **بالمقصودِ لا بالعرضِ**: لولاهُ لَكتبَتِ الدفعةُ الطائرةُ نقطتَها فوقَ
  الإرجاعِ بعدَ `COMMIT` فبَطَلَتِ الإعادةُ صامتةً — وهوَ الحدُّ الذي رفعتْهُ هذه
  المراجعةُ، مُبرهَنٌ باختبارِ السباقِ المُصمَّمِ لا بوعدٍ في هذا النصِّ. ولا
  تغييرَ في الجوابِ: يبقى **202** بالشكلِ نفسِهِ — إنّما تأخَّرَ وصولُهُ مدةَ
  دفعةٍ جاريةٍ على الأكثرِ.
- **ولا جسمَ:** `{"reason": "…"}` يُرَدُّ **400** لا يُتجاهَلُ صامتاً — منادٍ يظنُّ
  أنَّهُ سجَّلَ سبباً يعتقدُ أنَّ في الدفترِ ما ليسَ فيهِ (سابقةُ §2.3ب).
- **والرفضانِ بكودَينِ:** `DELIVERY_RELAY_DEAD_LETTER_NOT_FOUND` (**404**) و
  `DELIVERY_RELAY_DEAD_LETTER_NOT_POISONED` (**409** · **والحالةُ المقروءةُ في
  نصِّ الرسالةِ** لأنَّ عقدَ خطأِ التوصيلِ لا ينشرُ `details`). ودمجُهُما كانَ
  يجعلُ مُشغِّلاً يُطارِدُ مُعرِّفاً صحيحاً وزميلُهُ سبقَهُ قبلَ ثانيةٍ.
- **و500 حينَ لا منفذَ إعادةٍ مُركَّبٌ:** ادّعاءُ إعادةٍ لم تقعْ أسوأُ من رفضٍ.
- **وحدودٌ مُعلَنةٌ:** لا قفلَ موزَّعٌ (دفعةٌ طائرةٌ قد تُقدِّمُ النقطةَ فوقَ
...
| ~~**إعادةُ معالجةِ صفٍّ `poisoned`** (`re-process`/`replay`)~~ **رُفِعَ في المراجعةِ 22/N** | [ADR-026 §4.24](../15-decisions/ADR-026-store-orders-and-delivery-boundary.md) · §2.3د · [دليلُ التشغيلِ](../14-runbooks/RELAY_POISONED_EVENTS.md) §4 — `POST …/requeue` يرفعُ نهائيّةَ الحكمِ ويُرجِعُ نقطةَ التقدُّمِ **في معاملةٍ واحدةٍ**، بصلاحيّةٍ مفصولةٍ و`202` تقولُ «قُبِلَ» لا «عولِجَ» · **والسؤالُ الذي طُرِحَ هنا أُجيبَ لا أُلغِيَ**: مَن يُقرِّرُ أنَّ الفسادَ زالَ هوَ **إنسانٌ** يُنادي المسارَ (لا تنبيهٌ آليٌّ)، وإعادةُ تطبيقِ حدثٍ قديمٍ على حالةٍ تقدَّمَت يمنعُها حرسُ الترتيبِ القائمُ في المُرحِّلِ لا هذا المسارُ · **وحدودُهُ مُعلَنةٌ** (لا قفلَ موزَّعٌ · لا جُملةٌ · محاولةٌ واحدةٌ لمُستنفِدٍ) |
