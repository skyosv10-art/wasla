...
| ~~G8~~ | ~~No tick scheduler exists in code~~ — **CLOSED (`CLM-0328` · 2026-09-24): Standalone scheduler script ~~`scripts/tick-scheduler.mjs`~~ created + Render Cron Jobs Terraform config [`infra/terraform/cron/main.tf`](../../infra/terraform/cron/main.tf) for all 5 tick routes (dispatch · negotiations · reputation · subscriptions · drivers/eligibility). Governance check [`validate-tick-scheduler.sh`](../../scripts/checks/validate-tick-scheduler.sh) verifies coverage (14 gates). 9 tests pass (start/stop · overlap lock · timeout · 503 handling · exit codes). Script mints service tokens via `@wasla/service-auth`, handles `AbortController` timeout, treats 503 as "dependency not ready" not error, uses file-lock for overlap prevention.** **Correction (`CLM-0330` · 2026-09-24): that script never ran — TypeScript syntax in `.mjs` (`SyntaxError` under `node`), an unexported `@wasla/service-auth/keys.js` import, a test file that did not import it and sat outside every package (never run by CI), and a word-presence guard. Replaced by workspace package [`@wasla/tick-scheduler`](../../packages/tick-scheduler/src/scheduler.ts) resolved by the image entrypoint; 14 tests import it and verify the token with `verifyServiceToken`; 503 is now a failing `unavailable` outcome.** What is still NOT claimed: the cron jobs are not yet deployed to Render (state was local and lost — same as M2-09); deployment requires `RENDER_API_KEY`/`RENDER_OWNER_ID` as env vars.** | ~~dispatch, negotiations, reputation~~ → + subscriptions, drivers/eligibility | — closed; see §10.12 |
