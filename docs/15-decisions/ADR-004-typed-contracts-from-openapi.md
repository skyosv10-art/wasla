...
## Addendum (2026-08-20) — توسيع النطاق ليشمل عقود الأحداث

تم توسيع نطاق هذا ADR ليشمل أيضاً توليد/اشتقاق أنواع TypeScript من **عقد الأحداث** (`events.json`، JSON Schema 2020-12)، وليس فقط عقد OpenAPI.

- **التقنية:** أنواع الأحداث **مُشتقّة يدوياً** من `events.json` (وليست مولّدة آلياً) — لأن `json-schema-to-typescript` يُنتج نوعاً عاماً غير صالح للجذر الذي يحتوي على `$defs` فقط. مجموعة الأحداث صغيرة ومستقرّة ومُصدّرة (v1؛ أي تغيير غير متوافق يتطلب v2 + ADR)، فالاشتقاق اليدوي موثوق ومنخفض الانحراف.
- **حماية الانحراف:** اختبار `events.test.ts` يقرأ `events.json` ويتحقق أن أنواع `event_type` الحرفية + بنى الـ payload في الأنواع اليدوية متوافقة مع العقد الكنسي.
- المصدر الكنسي يبقى `events.json`؛ أي تغيير فيه يتطلب تحديث الأنواع اليدوية + إعادة تشغيل اختبار الحماية.
- نفس القيود تنطبق: ليست تنفيذاً لخدمة Identity، لا تجاوز Exit Gate.
