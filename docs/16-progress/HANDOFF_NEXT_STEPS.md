...
## 18. Phase 10 (Driver Subscription & Referral) — قيد التنفيذ 🔄 · **MR 1/6 من ستّ مدفوعة** · وما تفعله المراجعةُ التالية بالحرف
...
|---|---|---|
| قرارُ الحدّ | [ADR-015](../15-decisions/ADR-015-driver-subscription-entitlement-ledger-and-derived-referral-rewards.md) | تسعةُ قراراتٍ مُثبَّتة |
...
| حرّاسُ الانحراف | `packages/contracts/subscription/src/__tests__/` | **26/26 ناجح · 0 فاشل** |
| وثيقةُ الحاويات | [CONTAINERS §4.7](../02-architecture/CONTAINERS.md) | تسعُ حدودٍ محروسة |

**المقيسُ محلّيّاً:** المستودعُ كاملاً **2882 ناجحاً · 0 فاشل** (الأساسُ 2856 + 26) · `pnpm -r typecheck` **نظيفٌ على 35 مشروعاً**. **القياسُ محلّيٌّ لا في CI**: حصّةُ الدقائقِ مستنفدةٌ (`ci_quota_exceeded`) — انحرافٌ مُعلَنٌ وفق §2-أ.
...
4. **`community` أرضيةُ امتيازٍ لا عقوبة** — ولا اقترانَ لها بـ`suspend`/`block`/`ban`؛ حارسُ `boundary.test.ts` يفشل إن ظهر.

### 18.3 MR 2/6 — طبقةُ مجالٍ نقيّةٌ بلا إدخال/إخراج (النطاقُ بالحرف)

**اكتب في `services/subscriptions/src/domain/` فقط**، بلا `pg` ولا HTTP ولا `Date.now()` (الساعةُ **مُحقونة**، على سابقة Phase 09):

1. **`deriveState(periods, now)`** → إحدى الأربع، واختبارُها على حدودِ اليومِ نفسِه لا على «اليوم».
2. **`assertTransition(from, to)`** يقرأ `SUBSCRIPTION_ALLOWED_TRANSITIONS` من الحزمة — **لا جدولَ ثانياً في الخدمة**، وإلّا صار عندك حقيقتان.
3. **بذرةُ خطّةِ الإطلاق** `saudi-driver-monthly` v1: تجربة 14 · مدّة 30 · سماح 7 · مكافأة 30 يوماً · **5 وقائعَ مؤهِّلة** · نافذة 30 يوماً (دَينٌ مُعلَنٌ من MR 1/6، بند 2).
4. **`qualifyReferral(facts, plan, now)`** يُرجِع إمّا تأهيلاً أو **رمزَ رفضٍ من القائمةِ المُغلَقة** `REFERRAL_REJECTION_REASONS` — لا نصَّ حرّاً.
5. **`applyReferralReward()`** يُنتج **مُدّةً واحدةً** بـ`source='referral_reward'`؛ وحدةُ الحمايةِ `ux_referral_rewards_referral`.

**بوابةُ قبولِ MR 2/6:** كلُّ حافّةٍ في جدولِ الانتقالاتِ لها اختبارٌ يُثبتها **وآخرُ يُثبت رفضَ نقيضِها**؛ ولا استيرادَ واحداً لـ`pg` أو `fastify` داخل `domain/` (أضِف `purity.test.ts` على نمطِ خدمةِ السمعة).

### 18.4 بقيّةُ الطريقِ في هذا الطور

| المراجعة | المدى | البوابة |
|---|---|---|
| 3/6 | استمراريّةُ Postgres + مُهجِّرٌ يُطبّق `schema.sql` فعلاً | المخطَّطُ مُطبَّقٌ على محرّكٍ حقيقيّ لا مقروءٌ نصّيّاً (دَينُ MR 1/6، بند 3) |
| 4/6 | طبقةُ HTTP على **8093** ضدّ الواجهةِ المجمَّدة | كلُّ عمليّةٍ من الاثنتَي عشرةَ لها اختبارُ عقدٍ برموزِ حالتِها |
...
### 18.5 الدَّينُ المُعلَنُ المنقولُ (اقرأه قبل أن تبني عليه)

1. **لا بذرةَ خططٍ في `schema.sql`** — مؤجَّلةٌ إلى MR 2/6 لأنّ `INSERT` بلا مُهجِّرٍ يُشغّله سطرٌ ميّت.
2. **لا Postgres في بيئةِ التنفيذ** — المخطَّطُ محروسٌ نصّيّاً؛ أوّلُ تطبيقٍ في MR 3/6.
3. **الحرّاسُ نصّيّةٌ لا دلاليّة** — تمنع تسريبَ حدٍّ في العقد، ولا تُثبت صحّةَ الاشتقاق.
