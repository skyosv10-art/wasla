...
الاختبارات:       **2328 اختباراً** (+1 متروك بقصد) في **142 ملفاً** — لقطة 2026-08-23 بعد Phase 08 · MR 2/6
...
## 14. Phase 08 (Negotiation & Chat) — قيد التنفيذ 🔄 (بدأت 2026-08-23) · MR 2/6 مدفوعة من ستّ
...
### ما يجب أن يُقرأ أولاً قبل MR 3/6

1. [NEGOTIATION_CORE_DOMAIN.md](../03-domain/NEGOTIATION_CORE_DOMAIN.md) كاملاً — هي عقد الطبقة التي ستُسنِدها إلى Postgres، و§1 فيها يقول المعيار صريحاً: **لا تعديل في `src/use-cases/` عند إسناد المنافذ**.
2. `services/negotiations/src/infrastructure/in-memory.ts` — **مواصفة المستودعات المُنتظَرة**: كلّ قاعدة يرفعها هذا الملفّ باسمها يجب أن ترفعها Postgres بنفس الاسم، و[ORDER_PERSISTENCE.md](../02-architecture/ORDER_PERSISTENCE.md) هو النمط المُتَّبع (مرآة Drizzle + حارس انحراف مرآة↔DDL + وحدة عمل واحدة).
3. `services/negotiations/src/__tests__/constraints.test.ts` — **الـ24 قاعدة مُسمّاة** هي ما يجب أن يُعاد إنجاحه على قاعدة حقيقيّة في وظيفة `negotiations-db-integration`.
