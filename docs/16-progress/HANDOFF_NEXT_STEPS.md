...
## 18. Phase 10 (Driver Subscription & Referral) — قيد التنفيذ 🔄 · **MR 2/6 من ستّ مدفوعة** · وما تفعله المراجعةُ التالية بالحرف
...
### 18.5 MR 3/6 — استمراريّةُ Postgres والمُهجِّر (النطاقُ بالحرف)

**اكتب في `services/subscriptions/src/{db,migrations}/` فقط، ولا تلمس `domain/` إلّا بإضافةٍ يبرّرها اختبار:**

1. **مُهجِّرٌ يُطبّق `schema.sql` على محرّكٍ حقيقيّ** — لا قراءةً نصّيّةً له. (دَينُ MR 1/6، بند 3.)
2. **بذرةُ صفوفِ الخطّةِ تُكتَب من `plans.ts` نفسِه** — لا `INSERT` بأرقامٍ مكتوبةٍ ثانيةً. نسخةٌ ثانيةٌ للأرقامِ تعني خطّةً في القاعدةِ تخالف الخطّةَ في الكودِ بلا اختبارٍ يكشف ذلك.
3. **مخزنٌ append-only**: `insertPeriod` و`listPeriods(driver)` و`insertTransition` — **ولا `UPDATE` على حالةٍ** بحال؛ والاشتقاقُ يبقى في `domain/state.ts` يُنادى بعد القراءة.
4. **مطابقةُ المُهيئَين**: صفوفُ القاعدةِ تُحوَّل إلى `Period` بحقولٍ **مُسمّاةٍ صراحةً** لا بنسخٍ آليٍّ، على سابقة MR 3/6 في الطور 09 (§16-ز).
5. **`purity.test.ts` يُحدَّث لا يُخترَق**: أضِف ملفَّ القاعدةِ إلى `DB_AWARE_FILES` **بالاسم** — والاختبارُ الموجَبُ سيلزمك بذلك تلقائياً.

**بوابةُ قبولِ MR 3/6:** المخطَّطُ مُطبَّقٌ فعلاً على محرّكٍ (أو تُعلَن استحالةُ البيئةِ صراحةً كدَينٍ لا كنجاح) · صفوفُ الخطّةِ في القاعدةِ **تُطابق `LAUNCH_PLAN` حقلاً بحقلٍ** باختبارٍ يقرأ الاثنَين · ولا `UPDATE` واحدٌ على `subscription_periods`.

### 18.6 بقيّةُ الطريقِ في هذا الطور

| المراجعة | المدى | البوابة |
|---|---|---|
| ~~2/6~~ | ~~طبقةُ مجالٍ نقيّة~~ | ✅ **تمّت** — 100 اختباراً · [!72](https://gitlab.com/uxxxu/wasla/-/merge_requests/72) |
| 3/6 | استمراريّةُ Postgres + مُهجِّرٌ يُطبّق `schema.sql` ويكتب بذرةَ الخطّة | 18.5 أعلاه |
| 4/6 | طبقةُ HTTP على **8093** ضدّ الواجهةِ المجمَّدة + `POST /subscriptions/tick` | كلُّ عمليّةٍ من الاثنتَي عشرةَ لها اختبارُ عقدٍ برموزِ حالتِها |
...
### 18.7 الدَّينُ المُعلَنُ المنقولُ (اقرأه قبل أن تبني عليه)

1. **لا Postgres في بيئةِ التنفيذ** — المخطَّطُ محروسٌ نصّيّاً؛ أوّلُ تطبيقٍ في MR 3/6.
2. **بذرةُ الخططِ في الكودِ لا في القاعدة** — `plans.ts` هي البذرةُ اليوم، والمُهجِّرُ يكتبها في 3/6.
