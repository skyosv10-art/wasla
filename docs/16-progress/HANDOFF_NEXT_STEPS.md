...
## 19. Phase 11 (Marketplace Foundation) — **جارية 🔄 · المراجعتان 1/6 و2/6 مدفوعتان** (العقودُ وحزمةُ الأنواعِ وADR-016 · ثمّ طبقةُ المجالِ النقيّة)
...
### 19.3 المراجعاتُ الأربعُ الباقية (الترتيبُ مُلزم)

- ~~**2/6** طبقةُ المجالِ النقيّةُ في `services/marketplace/src/domain/`~~ — ✅ **مدفوعة 2026-08-24** (التفصيلُ في §19.5 أعلاه). والباقياتُ أربعٌ:
- **3/6** الاستمراريّةُ في `src/db/`: مُهاجرةٌ تُطبّق `schema.sql` حرفاً · مستودعاتٌ · اختباراتٌ على Postgres حقيقيّ (المنفذ 55432 · [الدليل](../14-runbooks/LOCAL_POSTGRES_FOR_TESTS.md)).
...
1. **حصّةُ CI ما زالت مستنفدة** (`ci_quota_exceeded`): كلُّ أرقامِ الطورِ **محلّيّةٌ** — 46/46 في `packages/contracts/marketplace` و**125/125** في `services/marketplace`، والمستودعُ كاملاً **3263 ناجحاً** (+125 بالضبط على خطِّ الأساس 3138) و`pnpm -r typecheck` نظيف. انحرافٌ **مُعلَنٌ** لا مُخفى، ويُعاد قياسُه في CI أوّلَ ما تُستعاد الحصّة.
