...
## 19. Phase 11 (Marketplace Foundation) — **جارية 🔄 · المراجعة 1/6 مدفوعة** (العقودُ وحزمةُ الأنواعِ وADR-016)

### 19.1 ما وقع في هذه المراجعة بالضبط
...
### 19.3 المراجعاتُ الخمسُ الباقية (الترتيبُ مُلزم)

- **2/6** طبقةُ المجالِ النقيّةُ في `services/marketplace/src/domain/`: جدولا الانتقال · اشتقاقُ الحالةِ من الدفتر · دالّةُ الظهور · حدودُ `slug` والأدوار — بلا قاعدةِ بياناتٍ ولا HTTP.
...
1. **حصّةُ CI ما زالت مستنفدة** (`ci_quota_exceeded`): كلُّ أرقامِ هذه المراجعةِ **محلّيّةٌ** (46/46 في الحزمة · المستودعُ كاملاً **3138 ناجحاً · 1 مُتخطّىً** · `pnpm -r typecheck` نظيف). أوّلُ خطٍّ يمرّ على `main` يجب أن يُثبِّت الأرقامَ ويُحدَّث هذا البند.
2. **لا وظيفةَ CI جديدةً في هذه المراجعة** بقصد: `build-test` تغطّي كلَّ الحزم، وحزمةُ العقدِ تدخل تحتها بلا تعديل.
