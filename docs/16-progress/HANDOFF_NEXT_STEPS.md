...
## 18. Phase 10 (Driver Subscription & Referral) — قيد التنفيذ 🔄 · **MR 4/6 من ستّ مدفوعة** · وما تفعله المراجعةُ التالية بالحرف
...
| **5/6** | مستهلكُ وقائعِ السمعة + ناشرُ الصادر + `subscription_idempotency` + تأهيلُ الإحالةِ ومكافأتُها | تسليمٌ مرّةً واحدةً مُثبَتٌ (دَينُ MR 1/6، بند 5) · وإعادةُ المفتاحِ تُعيد **نفسَ البايتات** لا 409 · وسدُّ دَينِ 18.9-6 |
...
6. ~~**دَينُ MR 3/6 على 4/6**: لا صفَّ `subscriptions` مُتحقِّقاً ولا معاملةً~~ ✅ **سُدَّ في MR 4/6** (18.6-أ). **والباقي منه يملكه 5/6**: لا **حلقةَ إعادةٍ** عند تزامنِ تسلسلِ الانتقالات (القيدُ المتفرّدُ يُنتج خطأً لا صفّاً مكسوراً) · و`subscriptions-db-integration` مُضافةٌ ولم تركض (18.5-ج).
7. **دَينٌ جديدٌ من MR 4/6**: لا `projection.integration.test.ts` (مساواةُ المؤشِّرِ للاشتقاقِ مُثبَتةٌ **عَبَر HTTP** لا على المخزنِ مباشرةً) · و**ذرّيّةُ المعاملةِ غيرُ مُثبَتةٍ بفشلٍ مُصطنعٍ في منتصفِها** (لا `TransactionProbe`) · و`payment_reference` يُتحقَّق **4..64** بينما العقدُ يقول `minLength: 1` فيلزم ملحقٌ في 5/6 · وتعدادُ `ErrorResponse.code` ناقصٌ **أربعةَ رموزٍ** تُصدرها الخدمةُ فعلاً ويحرسُ غيابَها اختبارُ فجوةٍ معروفة.
