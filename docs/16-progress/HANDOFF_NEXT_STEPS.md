...
المتبقّي:         Phase 04 (MR 5/6: ربط البوت · MR 6/6: بوابة الخروج E2E · §9) → Phase 24 (انظر §3 للمسار الكامل، و§7 لما تُسلّمه Phase 03).
...
| 4 | طبقة HTTP | `src/http/{requests,errors,app,server}.ts` — تطبيق Fastify على المنفذ **8086** + عشرة مسارات + تخطيط الأكواد الثمانية عشر إلى حالات HTTP (بلا كود جديد) + `/health` بحالتيه + محوّلا HTTP للهوية والجغرافيا + `x-request-id` → `trace_id` | ✅ **مدمجة ([!MR_IID](https://gitlab.com/uxxxu/wasla/-/merge_requests/MR_IID))** — 34 اختبار `app.inject` · [CUSTOMER_HTTP.md](../04-api/CUSTOMER_HTTP.md) |
| 5 | البوت | ربط `bots/customer-bot` بالخدمة (ملف · مكان محفوظ · إنشاء طلب) مع **الحفاظ على حياد القناة** (ADR-007): البوت لا يعرف مجال العميل، والمجال لا يعرف Telegram | ⬜ **التالية** |
...
**ما يجب أن يعرفه من يبدأ MR 5/6 قبل أن يكتب سطراً** (النقاط 3..7 قيود دائمة لا تنتهي بدفعة، والنقطتان 1 و2 حُسمتا في MR 4/6):
