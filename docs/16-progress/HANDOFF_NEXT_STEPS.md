...
## 15. الخطوة التالية — **Phase 09 (Reputation + Fraud)** · ما يجب أن يُقرأ أولاً
...
## 16. Phase 09 (Reputation + Fraud) — قيد التنفيذ 🚧 (2026-08-23) · MR 4/6 من ستّ مدفوعة
...
### 16-ي. المراجعة التالية — MR 5/6: مستهلكُ حدثَي الطلب وناشرُ صندوق الصادر

**الحدّ:** لا تعديلَ في `domain/` ولا `use-cases/` ولا `contracts/*` — كما في 3/6 و4/6.

1. **مستهلكٌ لحدثَي محرّك الطلب** `order.status_changed` و`order.assignment_resolved` يُحوّلهما إلى وقائع
   عبر **نفس** `recordFact` (أو `POST /reputation/facts`)، و**`source_event_id` هو UUID الحدث** لا مُعرّفاً
   مُركّباً — وإلّا رُفض بـ`22P02` **بلا اسمِ قيد** ([REPUTATION_PERSISTENCE §8-هـ](../02-architecture/REPUTATION_PERSISTENCE.md)).
2. **التسليم at-least-once فالتفرّدُ قيدٌ في القاعدة** (`ux_reputation_facts_source`): إعادةُ نفس الحدث
   ⇒ `duplicate: true` بلا نقطةٍ ثانيةٍ وبلا حدثٍ ثانٍ. والاختبارُ يقيس **عددَ الأحداث المنشورة** لا الرمزَ وحده.
3. **ناشرٌ يُرحّل `reputation_outbox`** (دَينُ الأطوار 06 · 07 · 08 نفسُه): يقرأ غيرَ المنشور، يُسلّم، ثمّ
   يُعلّم — وفشلُ التسليم لا يُبطل الكتابة، وإعادةُ المحاولة لا تُنتج نشرتين لنفس الصفّ.
4. **`x-request-id` يمرّ كما هو** من الحدث إلى الواقعة إلى الحدث الصادر، فيبقى الأثرُ متّصلاً بين خدمتين.
5. **لا مؤقّت:** تقديمُ الزمن بـ`POST /reputation/tick` وحده، ومُشغّلُه خارجُ الخدمة.

**ولا تلمس:** حدَّ المعاملة · ولا الحرّاسَ الأربعة (`constraints` · `purity` · `schema-drift` · `http-drift`)
تضييقاً — التوسيعُ مسموحٌ بشرط اختبارٍ موجَب، والتضييقُ يحتاج ADR.
