...
>
> **Last Updated:** 2026-08-22 (**Phase 05 · MR 4/6 مدفوعة** — طبقة HTTP لخدمة السائقين على **8090**: ثلاثة عشر مساراً و`/health` بحالتيه، والمعالجات تستقبل **مقبس معاملة وحده** فلا أحد منها في موضع يستطيع فيه فتح معاملة. وثلاثة عيوب عقد صُحِّحت قبل كتابة سطر مسار (`HealthStatus.last_tick_at` الغائب · `VehiclePatch.status` الذي كان يَعِد بـ`active` وهي مرفوضة بـ400 منذ MR 2/6 · `ErrorResponse.error.details` بلا شكل)، وحقلٌ زائد حُذف من `mappers.ts`. و**عيبٌ في تنفيذنا أسقطه اختبار**: المركبات والوثائق كانت تُعيد السطر الموجود لمفتاح مُعاد **بحمولة مختلفة** فتُجيب 200 عن مركبة ليست المطلوبة — الآن يُستدعى المجال دائماً ويبقى قرار الإعادة له. و`GET` القوائم والأهليّة تُجيب **404 صريحاً** لا قائمة فارغة ولا حكم `unknown`. **64 اختبار HTTP** جديداً منها حرّاس انحراف تفرض الاتجاهين: لا عملية عقد بلا مسار ولا مسار غير موثَّق — [DRIVER_HTTP.md](../04-api/DRIVER_HTTP.md) · سابقاً **MR 3/6** — استمرارية السائق على Drizzle/Postgres: تسعة مُهيّئات تخزين وراء المنافذ نفسها بوحدة عمل واحدة تجعل العمليّة كلّها ترتكز أو تتراجع، **ولم يتغيّر أي ملفّ تحت `src/use-cases/`**؛ وكُشف منفذٌ بلا جدول فأُضيف `driver_idempotency` بعرض 8..192 لا 8..128 لأنّ المفتاح مُنطَّق؛ و28 حارس انحراف + 79 اختبار تكامل في `drivers-db-integration` — [DRIVER_PERSISTENCE.md](../02-architecture/DRIVER_PERSISTENCE.md) · سابقاً **MR 2/6** — طبقة مجال السائق النقيّة وحاسب الأهليّة: 69 اختباراً بلا قاعدة وبلا HTTP، والقاعدة الحاكمة **لا تغيّر حالة بلا إعادة قرار** فكلّ كتابة تنتهي عند `recomputeEligibility` وحدها. و**تصحيح على §13:** الخطّة كانت تقول «السبب الأوّل يفوز» نقلاً عن مجال المطابقة، ونُقضت عند التنفيذ إلى **«كلّ الأسباب لا أوّلها»** — المطابقة تُقيّم آلاف الصفوف لمُوزّع فيكفيها سبب واحد، ونواة السائق تُجيب **سائقاً واحداً عن نفسه** فجوابها قائمة عمل، وقائمةٌ جزئيّة تجعله يُصلح شيئاً ويُرفض مرّة أخرى؛ و`PROFILE_SUSPENDED` هو القاطع الوحيد لأنّ الوثائق لا ترفع إيقافاً. التفصيل في [DRIVER_CORE_DOMAIN.md](../02-architecture/DRIVER_CORE_DOMAIN.md) · سابقاً 2026-08-22 (**Phase 07 = Completed** — MR 6/6 بوابة الخروج مدمجة وبها أُغلق الطور: `packages/dispatch-e2e` تُقلع ستّ خدمات بساعة واحدة مُحقونة وتُثبت المسار الكامل وثلاث نهايات غير سعيدة، ووظيفة `dispatch-exit-gate-e2e` تُعيدها على Postgres — والبوابة أسقطت عيباً كان يجعل **كل** قبول سائق يُرفض 422. التفصيل §11 و[PHASE07_EXIT_GATE_E2E.md](../12-testing/PHASE07_EXIT_GATE_E2E.md). **التالي: Phase 05 (Driver Core) أو Phase 08 (Negotiation & Chat) — §12**)
>
...
```text
المرحلة الحالية: **Phase 05 (Driver Core) — قيد التنفيذ** · MR 1/6 → 5/6 مدفوعة (العقود + ADR-012 · ثمّ طبقة المجال النقيّة وحاسب الأهليّة بـ69 اختباراً · ثمّ استمرارية Drizzle/Postgres بوحدة عمل واحدة **بلا تغيير في `src/use-cases/`**، بـ28 حارس انحراف و79 اختبار تكامل في `drivers-db-integration` · ثمّ طبقة HTTP على **8090** بثلاثة عشر مساراً و64 اختبار HTTP، وبثلاثة تصحيحات عقد وعيبِ إعادةٍ أسقطه اختبار · ثمّ **المنافذ الصادرة الحقيقية** إلى المطابقة 8088 والجغرافيا 8081 وواجهة بوت السائق، وفيها **تقاعُد 502** وإصلاح عيبٍ كان يُسقط كتابةً محليّة ناجحة عند عطب المطابقة وعيبِ مُشغِّلٍ كان يُلغي تدفقات بوت العميل المنشور) · **التالي MR 6/6 — بوابة خروج المرحلة** (§13)
...
الاختبارات:       **2112 اختباراً على 28 مشروع عمل** (+1 متروك بقصد) في 128 ملفاً — لقطة 2026-08-22 بعد Phase 05 · MR 5/6
                 (ومنها 194 لخدمة السائقين في 13 ملفاً · 59 لعقودها · 27 لبوت السائق) · والتفصيل التاريخي أدناه من الطور 07:
...
Phase 05 Driver Core ...................... 🚧 قيد التنفيذ — MR 1/6 → 5/6 مدفوعة · بقيت 6/6 (بوابة الخروج) · §13
...
## 13. Phase 05 (Driver Core) — قيد التنفيذ 🚧 (بدأت 2026-08-22) · MR 1/6 → 5/6 مدفوعة · **التالي 6/6 (بوابة الخروج)**
