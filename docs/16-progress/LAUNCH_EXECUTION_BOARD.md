...
| **أول تحقّق فعلي منذ توقّف CI** | 2026-08-25 — شُغّلت **وظائفُ قاعدةِ البيانات الثمانَ عشرةَ كلُّها** محليّاً مقابل PostgreSQL 15.6 (ثنائيّاتٌ محمولةٌ تعمل بلا صلاحيّاتِ root، قاعدةٌ مستهلَكةٌ منفصلةٌ لكلِّ وظيفة، مصادقةٌ محليّة — **بلا أيِّ سرٍّ ولا أيِّ قاعدةٍ مُستضافة**). النتيجة: **17/18 خضراء، 450 اختباراً**، والفاشلةُ الوحيدةُ `drivers-db-integration`. المسارُ منها إلى الخُضرة: 27 فشلاً ← **7** بعد `M0-02` ← **1** بعد `M0-09` ← **0** بعد `M0-10`. أي **18/18 خضراء** الآن. |
...
| M0-11 | إصلاح تقلّبِ بوّابةِ الحوكمة (SIGPIPE في `validate-launch-board.sh`) | Program / DevEx | — | Ready for Gate | 40 تشغيلاً بلا تقلّب + حالةُ انحدار | **مُكتشَفٌ بالتشغيل، والبوّابةُ نفسُها هي المصاب.** السطرُ 74 كان `printf … | grep -Fxq`: تحت `pipefail` يخرج `grep -q` عند أوّلِ تطابقٍ فيُغلقُ الأنبوب، فيموتُ `printf` بـSIGPIPE، فتصيرُ حالةُ الأنبوبِ 141 وتُقرأ «لا تطابق» — فيُرفَض معرِّفٌ **موجودٌ** على اللوحة. مُثبَتٌ تجريبيّاً: **66 نتيجةً خاطئةً من 300** قبل العلاج، **0 من 300** بعده. عُولج بـ`<<<` بلا أنبوب، وأُضيفت الحالةُ 15 (40 تشغيلاً متعاقباً يجب أن تنجح كلُّها) لأنّ عيباً احتماليّاً لا تكفيه حالةٌ واحدة. |
| M0-12 | فكُّ ارتباطِ حزمةِ إثباتِ الحوكمة بصفٍّ بعينِه | Program / DevEx | M0-11 | Not Started | 15/15 على أيِّ فرعٍ وبأيِّ حالةِ سجل | **عيبُ تصميمٍ في نظامِ الحوكمةِ نفسِه، مُكتشَفٌ بالتشغيل.** `test-governance.sh` يُعدِّل الصفَّ `CLM-0001` والفرعَ `chore/unified-roadmap-governance` بالاسم، وكلاهما انتقلَ إلى قسمِ «المحرَّرة» وحُذف عند دمجِ [MR !81](https://gitlab.com/uxxxu/wasla/-/merge_requests/81) — فصارت 4 حالاتٍ من 15 تفشل أو تصير عابثةً: 3 تتوقّع رفضاً فتأتي قبولاً (لأنّ الصفَّ المُشوَّهَ لم يعد `Active` فيتجاوزه المدقّق)، وواحدةٌ تفشل لغيابِ حجزٍ نشطٍ لذلك الفرع. أي أنّ الحزمةَ **تتلفُ بمجرّدِ تحريرِ أوّلِ حجز**، وهو ما يفعله سيرُ العملِ الطبيعي. العلاجُ: تبني الحزمةُ سجلَّ حجزٍ صناعيّاً مستقلّاً في مجلّدٍ مؤقّتٍ بدلَ تعديلِ ما يقع في المستودعِ الحقيقي. **ملاحظةٌ للأمانة:** نتيجةُ «14/14» المُعلَنةُ عند MR !81 كانت صحيحةً لحظتَها لأنّ `CLM-0001` كان `Active` حينئذٍ، ولم تكن لتبقى صحيحةً بعدَه. |
| M0-09 | تصحيح تأكيدات القيود بعد ترقية Drizzle | Drivers / QA | M0-02 | Ready for Gate | `drivers-db-integration` 7 فشول ← 1 (والباقي هو M0-10 وحدَه) | **مُكتشَفٌ بالتشغيلِ الفعلي، لم يكن على اللوحة.** `drizzle-orm@0.45` يلفّ خطأَ Postgres في `DrizzleQueryError` بمفاتيحٍ `query,params,cause` فقط، فصار `constraint` على `cause` لا على الخطأِ نفسِه؛ وستّةُ اختباراتٍ تؤكّد `rejects.toMatchObject({ constraint })` فتفشل رغمَ أنّ القيدَ **رفضَ فعلاً** (مُثبَتٌ بسبرٍ طبعَ الكائن). تغييرٌ في الاختباراتِ وحدَها. ويشمل خطأً حسابيّاً: `expect(key.length).toBe(151)` والصحيحُ **150** (`8+13+1+128`)، وحدُّ العمودِ 192. |
| M0-10 | إصلاح `ck_eligibility_log_reasons` — مصيدةُ `array_length` | Drivers / DB | M0-02 | Ready for Gate | **`drivers-db-integration` 79/79 — الوظيفةُ خضراءُ بالكامل** | **عيبُ إنتاجٍ حقيقيٌّ مُثبَتٌ بالتجربة، لم يكن على اللوحة.** القيدُ `to_state = 'eligible' OR array_length(reasons,1) >= 1` **لا يرفض شيئاً** للمصفوفةِ الفارغة: `array_length('{}',1)` = `NULL`، و`NULL >= 1` = `NULL`، وPostgres يعدُّ نتيجةَ `NULL` في `CHECK` **ناجحةً**. فيُقبَل حكمُ `ineligible` بلا سببٍ واحد — وهو الشيءُ الوحيدُ الذي كُتب القيدُ لمنعِه. العلاجُ `cardinality(reasons) >= 1` (تُعطي `0` للفارغة). يحتاج تعديلَ `contracts/schema.sql` + ترحيلاً، فبندٌ مستقلٌّ لا توسيعُ نطاق. |
