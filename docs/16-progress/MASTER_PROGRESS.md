...
| 01 | Identity Foundation | **In Progress** — عقود مدمجة + أنواع TS مولّدة (Contract First ~30%) | إنشاء مستخدم من Telegram وبقاء هويته مستقرة عبر تغيير Username | 01, 09, 10, 11, 12 | (1) **اختيار المكدّ التقني** لخدمة Identity (ADR منفصل) — معلّق. (2) تنفيذ فعلي ضد العقود + اختبارات Contract (consumer/provider). (3) shared runners لا يزال يعيق التحقق من CI (يؤثر على التحقق، لا على العقود/الأنواع). بدء العقود رغم العائق موثّق في [ADR-002](../15-decisions/ADR-002-begin-phase01-contracts-despite-shared-runners-blocker.md)؛ توسيع توليد الأنواع موثّق في [ADR-004](../15-decisions/ADR-004-typed-contracts-from-openapi.md). | 2026-08-20 | **[MR !2](https://gitlab.com/uxxxu/wasla/-/merge_requests/2) مدمج** (عقود OpenAPI/JSON Schema/DDL/errors) + **حزمة `@wasla/contracts-identity`**: أنواع TS مولّدة من OpenAPI عبر `openapi-typescript` + 6 اختبارات دخان (typecheck ✅ + test ✅). | اختيار المكدّ التقني (ADR) → تنفيذ ضد العقود → Contract tests → اجتياز Exit Gate |
