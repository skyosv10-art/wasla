...
|---|---|---|---|---|---|---|---|---|
| 00 | Repository Foundation | **Merged to main + build/test foundation added** — Exit Gate Pending (التحقق من CI) | CI passes، لا أسرار، جميع الفرق clone/build/test، Docs structure، Main branch protected، MR template active | جميع الفرق (Team 10 CI، Team 11 gates، Team 12 Telegram skeleton، Team 09 DB conventions) | (1) **GitLab shared runners غير متاحة للـ namespace المجاني** — تتطلب التحقق من namespace (User Settings → Billing) أو تشغيل runner خاص؛ بدونها لا تنفّذ أي وظيفة CI (مؤكد: pipelines على main فشلت فوراً بـ0 وظائف منذ 2026-08-19). (2) **بعد تفعيل الـ runners**: تشغيل pipeline على `main` للتحقق فعلياً من اجتياز CI (job `build-test` الجديد جاهز للعمل — يُنفّذ typecheck و test). (3) تفعيل `core.hooksPath` على نسخ المطورين. (4) [سياسة] حماية main تسمح لـMaintainers بالدفع المباشر — يُنصح بتشديدها إلى «No one». **تفاصيل التسليم الكاملة للجهة التالية: [HANDOFF_NEXT_STEPS.md](HANDOFF_NEXT_STEPS.md)** | 2026-08-20 | **[MR !1](https://gitlab.com/uxxxu/wasla/-/merge_requests/1) مدمج** (commit `cba9a75`) + **[MR !3](https://gitlab.com/uxxxu/wasla/-/merge_requests/3) مدمج** (تحديث الخارطة) + **أساس بناء عبر [ADR-003](../15-decisions/ADR-003-monorepo-tooling.md)**: pnpm 9 workspaces + tsconfig strict + Vitest + حزمة `@wasla/errors` مع اختبار دخان (3 اختبارات تجتاز) + job `build-test` في CI. التحقق محلياً: typecheck ✅ + test ✅ + scan-secrets ✅ + CI lint صالح (server-side). | تفعيل shared runners → تشغيل pipeline على `main` للتحقق من اجتياز CI (job `build-test`) → اعتماد Phase 00 = Completed (يحدّد W0) |
| 01 | Identity Foundation | **In Progress** — عقود مدمجة إلى main (Contract First ~25%) | إنشاء مستخدم من Telegram وبقاء هويته مستقرة عبر تغيير Username | 01, 09, 10, 11, 12 | (1) **اختيار المكدّ التقني** لخدمة Identity (ADR منفصل) — معلّق. (2) تنفيذ فعلي ضد العقود + اختبارات Contract (consumer/provider). (3) shared runners لا يزال يعيق التحقق من CI (يؤثر على التحقق، لا على العقود). بدء العقود رغم العائق موثّق في [ADR-002](../15-decisions/ADR-002-begin-phase01-contracts-despite-shared-runners-blocker.md). | 2026-08-20 | **[MR !2](https://gitlab.com/uxxxu/wasla/-/merge_requests/2) مدمج إلى main** (commit `a15985d`) — عقود Identity بمنهج Contract First (مستقلة عن المكدّ): [api.openapi.yml](../../services/identity/contracts/api.openapi.yml) (OpenAPI 3.0.3 صالح) · [events.json](../../services/identity/contracts/events.json) (JSON Schema صحيح) · [schema.sql](../../services/identity/contracts/schema.sql) (PostgreSQL DDL: users/links/history/recovery/outbox) · [errors.md](../../services/identity/contracts/errors.md) · فهرس المستهلك [packages/contracts/identity/](../../packages/contracts/identity/) | اختيار المكدّ التقني (ADR) → تنفيذ ضد العقود → Contract tests → اجتياز Exit Gate «إنشاء مستخدم من Telegram وبقاء هويته مستقرة عبر تغيير Username» |
