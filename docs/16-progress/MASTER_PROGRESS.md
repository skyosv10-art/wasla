...
| 02 | Geography & Localization Foundation | **In Progress** — عقود + ADR-006 مُدمجة (MR 2)؛ النواة/Postgres/HTTP/E2E تالية | المستخدم يغيّر موقعه دون حساب جديد، وكل Module يستعمل Geo IDs | 01, 02, 03, 06, 07, 08, 09 | **لا عوائق.** عقود Geography (13 جدولاً + 9 مسارات + حدثان + 12 كود خطأ) + ADR-006 مُتحقَّق محلياً (DDL على Postgres 18، typecheck 4 حزم، 15 اختباراً). | 2026-08-20 | [ADR-006](../15-decisions/ADR-006-geography-localization-stack-and-model.md) + `@wasla/contracts-geography` (15 اختباراً) + [MR !17](https://gitlab.com/uxxxu/wasla/-/merge_requests/17) (عقود + ADR). DDL يُطبَّق على Postgres (13 جدولاً). | MR 3 (pure core) → MR 4 (Drizzle+seed) → MR 5 (HTTP) → MR 6 (CI DB) → MR 7 (Exit Gate E2E) |
