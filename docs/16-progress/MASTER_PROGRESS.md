...
| 01 | Identity Foundation | **In Progress** — عقود + أنواع TS + اختيار المكدّ (ADR-005) + **نواة مجردة (MR 1: domain/ports/in-memory/use-cases + 15 اختباراً)** (~55%) | إنشاء مستخدم من Telegram وبقاء هويته مستقرة عبر تغيير Username | 01, 09, 10, 11, 12 | (1) ~~اختيار المكدّ~~ حُلّ (ADR-005). (2) ~~النواة المجردة~~ حُلّ ([MR !11](https://gitlab.com/uxxxu/wasla/-/merge_requests/11)). (3) طبقة Postgres/Drizzle (MR 2) — معلّقة. (4) طبقة Fastify HTTP (MR 3) — معلّقة. (5) اختبارات تكامل Postgres (MR 4) — معلّقة. (6) Exit Gate E2E (MR 5) — معلّقة. | 2026-08-20 | **[MR !2](https://gitlab.com/uxxxu/wasla/-/merge_requests/2) مدمج** (عقود) + `@wasla/contracts-identity` (13 اختباراً) + [ADR-005](../15-decisions/ADR-005-identity-service-implementation-stack.md) + **[MR !11](https://gitlab.com/uxxxu/wasla/-/merge_requests/11)** (نواة مجردة: domain/ports/in-memory/use-cases + 15 اختباراً، 31 إجمالي). | MR 2 (Drizzle/Postgres) → MR 3 (Fastify HTTP) → MR 4 (CI DB) → MR 5 (Exit Gate E2E) |
