...
|---|---|---|---|---|---|---|---|---|
| 00 | Repository Foundation | **Engineering work complete** — Exit Gate Pending (اجتياز CI على MR !9 فقط) | CI passes، لا أسرار، جميع الفرق clone/build/test، Docs structure، Main branch protected، MR template active | جميع الفرق (Team 10 CI، Team 11 gates، Team 12 Telegram skeleton، Team 09 DB conventions) | **shared runners مُفعّلة الآن** (تحقّق المالك من namespace 2026-08-20). ظهر فشل في job `build-test` (typecheck): `events.test.ts` يستعمل `node:fs`/`node:path`/`__dirname` دون `@types/node` مُعلَن — **صُلح عبر [MR !9](https://gitlab.com/uxxxu/wasla/-/merge_requests/9)** (إضافة `@types/node`، مُتحقَّق محلياً بتثبيت مُجمّد نظيف مُطابق لـCI). المعيار الوحيد المتبقّي: اجتياز pipeline فعلياً على MR !9. **تفاصيل التسليم: [HANDOFF_NEXT_STEPS.md](HANDOFF_NEXT_STEPS.md)** | 2026-08-20 | **[MR !1](https://gitlab.com/uxxxu/wasla/-/merge_requests/1) + [MR !3](https://gitlab.com/uxxxu/wasla/-/merge_requests/3) + [MR !4](https://gitlab.com/uxxxu/wasla/-/merge_requests/4) مدمجة** — بنية المستودع + القوانين + قالب MR + حماية main + فحص أسرار + doc-coverage + أساس بناء (pnpm+TS+Vitest) عبر [ADR-003](../15-decisions/ADR-003-monorepo-tooling.md) مع job `build-test` في CI. التحقق محلياً: typecheck ✅ + test ✅ (16 اختباراً) + scan-secrets ✅. **إصلاح build-test CI: [MR !9](https://gitlab.com/uxxxu/wasla/-/merge_requests/9)** (إضافة @types/node). | **دمج MR !9** → اجتياز pipeline على `main` → اعتماد Phase 00 = Completed (يحدّد W0) → بدء Phase 01 وفق [ADR-005](../15-decisions/ADR-005-identity-service-implementation-stack.md) |
