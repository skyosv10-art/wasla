...
| 02 | Geography & Localization Foundation | **In Progress** — عقود + ADR-006 (!17) + النواة المجردة (!18) + Drizzle/Postgres + Saudi seed (!19) + طبقة HTTP (!20) + تكامل CI لقاعدة البيانات (!21) مُدمجة؛ بوابة الخروج E2E تالية | المستخدم يغيّر موقعه دون حساب جديد، وكل Module يستعمل Geo IDs | 01, 02, 03, 06, 07, 08, 09 | **لا عوائق.** طبقة HTTP مُتحقَّق محلياً (typecheck 5 حزم، 96 اختبار وحدة منها 41 geography) وبتجربة تشغيل فعلية للخدمة (health + hierarchy مترجم + PUT 201 → 200 + history)؛ واختبارات تكامل Postgres الأربعة تعمل الآن في CI عبر وظيفة `geography-db-integration` (postgres:15، قاعدة wasla_geo_test). | 2026-08-20 | [ADR-006](../15-decisions/ADR-006-geography-localization-stack-and-model.md) + `@wasla/contracts-geography` (15) + [MR !17](https://gitlab.com/uxxxu/wasla/-/merge_requests/17) (عقود) + `@wasla/geography-service` pure core (25 اختباراً) + [MR !18](https://gitlab.com/uxxxu/wasla/-/merge_requests/18) (domain/ports/in-memory/use-cases/locale fallback) + [MR !19](https://gitlab.com/uxxxu/wasla/-/merge_requests/19) (Drizzle/Postgres + Saudi seed + 4 تكامل) + [MR !20](https://gitlab.com/uxxxu/wasla/-/merge_requests/20) (Fastify HTTP + تعيين الأخطاء + 16 اختبار app.inject) + [MR !21](https://gitlab.com/uxxxu/wasla/-/merge_requests/21) (وظيفة geography-db-integration في CI). | MR 7 (Exit Gate E2E + إغلاق Phase 02) |
