...
| 01 | Identity Foundation | **In Progress** — عقود مدمجة + أنواع TS (API + أحداث) (Contract First ~35%) | إنشاء مستخدم من Telegram وبقاء هويته مستقرة عبر تغيير Username | 01, 09, 10, 11, 12 | (1) **اختيار المكدّ التقني** لخدمة Identity (ADR منفصل) — معلّق. (2) تنفيذ فعلي ضد العقود + اختبارات Contract (consumer/provider). (3) shared runners لا يزال يعيق التحقق من CI (يؤثر على التحقق، لا على العقود/الأنواع). بدء العقود رغم العائق موثّق في [ADR-002](../15-decisions/ADR-002-begin-phase01-contracts-despite-shared-runners-blocker.md)؛ الأنواع (API + أحداث) موثّقة في [ADR-004](../15-decisions/ADR-004-typed-contracts-from-openapi.md) (مع ملحق توسيع نطاق الأحداث). | 2026-08-20 | **[MR !2](https://gitlab.com/uxxxu/wasla/-/merge_requests/2) مدمج** (عقود OpenAPI/JSON Schema/DDL/errors) + **حزمة `@wasla/contracts-identity`**: أنواع API مولّدة من OpenAPI (`openapi-typescript`) + أنواع أحداث مشتقّة من `events.json` (مع اختبار حماية انحراف) + 13 اختبار دخان (typecheck ✅ + test ✅). | اختيار المكدّ التقني (ADR) → تنفيذ ضد العقود/الأنواع → Contract tests → اجتياز Exit Gate |
