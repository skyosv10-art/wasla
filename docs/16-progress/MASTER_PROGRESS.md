...
| 03 | Telegram Channel Foundation | **In Progress (2026-08-20)** — MR 1/7 مدمجة: [ADR-007](../15-decisions/ADR-007-telegram-channel-adapter-isolation-and-stack.md) + عقود القناة + `@wasla/contracts-channel` | كل Bot يفتح Mini App، وAdapter قابل للاستبدال بـMock | 12, 01, 02, 03, 07 | **لا عوائق.** القرار المعماري محسوم: القناة طبقة توصيل لا خدمة (`packages/channel-core` محايد + `packages/telegram-adapter` وحده يعرف Bot API + `bots/*` جذور تركيب). مُهيّئات Postgres مؤجّلة إلى MR 5 (in-memory حتى ذلك الحين — موثّق في ADR-007). | 2026-08-20 | [ADR-007](../15-decisions/ADR-007-telegram-channel-adapter-isolation-and-stack.md) + `packages/channel-core/contracts/` (api.openapi.yml + events.json + schema.sql + errors.md) + `@wasla/contracts-channel` (34 اختباراً: أنواع مُولّدة + حراسة انحراف للأحداث وكتالوج الأخطاء + حراسة حدود ADR-007) + [MR !23](https://gitlab.com/uxxxu/wasla/-/merge_requests/23) | MR 2/7: نواة `channel-core` (نموذج المجال + المنافذ + مُهيّئات in-memory/Mock) — خطة السبع مراجعات كاملة في [HANDOFF §7](HANDOFF_NEXT_STEPS.md) |
