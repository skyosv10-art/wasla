...
|---|---|---|---|---|---|---|---|---|
| 00 | Repository Foundation | **Exit Gate Pending** (95% — خطوة خارجية واحدة) | CI passes، لا أسرار، جميع الفرق clone/build/test، Docs structure، Main branch protected، MR template active | جميع الفرق (Team 10 CI، Team 11 gates، Team 12 Telegram skeleton، Team 09 DB conventions) | (1) **GitLab shared runners غير متاحة للـ namespace المجاني** — تتطلب التحقق من namespace (User Settings → Billing) أو تشغيل runner خاص؛ بدونها لا تنفّذ أي وظيفة CI (مؤكد: pipelines على main فشلت فوراً بـ0 وظائف منذ 2026-08-19). (2) بعد توفّر الـ runners: اجتياز pipeline فعلياً على GitLab عبر [MR !1](https://gitlab.com/uxxxu/wasla/-/merge_requests/1). (3) تفعيل `core.hooksPath` على نسخ المطورين. (4) [سياسة] حماية main تسمح لـMaintainers بالدفع المباشر — يُنصح بتشديدها إلى «No one». **تفاصيل التسليم الكاملة للجهة التالية: [HANDOFF_NEXT_STEPS.md](HANDOFF_NEXT_STEPS.md)** | 2026-08-20 | بنية المستودع + القوانين + قالب MR + حماية main (متحقق منها) + سكربت فحص أسرار يعمل + doc-coverage تم اختباره E2E + CI config صالح (server-side lint valid) + وثيقة تسليم | تفعيل shared runners على GitLab، اجتياز pipeline على MR !1، ثم اعتماد Phase 00 = Completed وبدء Phase 01 (Identity Foundation) |
| 01 | Identity Foundation | **In Progress** (Contract First — ~20%) | إنشاء مستخدم من Telegram وبقاء هويته مستقرة عبر تغيير Username | 01, 09, 10, 11, 12 | (1) بدء عقود Identity رغم عائق shared runners موثّق في [ADR-002](../15-decisions/ADR-002-begin-phase01-contracts-despite-shared-runners-blocker.md). (2) بعد تفعيل shared runners ودمج [MR !1](https://gitlab.com/uxxxu/wasla/-/merge_requests/1): تنفيذ فعلي للعقود + اختبارات Contract + أمان + تكامل لاجتياز Exit Gate. | 2026-08-20 | عقود Identity بمنهج Contract First (مستقلة عن المكدّ): [api.openapi.yml](../../services/identity/contracts/api.openapi.yml) (OpenAPI 3.0.3 صالح) · [events.json](../../services/identity/contracts/events.json) (JSON Schema صحيح) · [schema.sql](../../services/identity/contracts/schema.sql) (PostgreSQL DDL: users/links/history/recovery/outbox) · [errors.md](../../services/identity/contracts/errors.md) · فهرس المستهلك [packages/contracts/identity/](../../packages/contracts/identity/) | تنفيذ العقود (اختيار المكدّ بـADR منفصل) + Contract tests + اجتياز Exit Gate «إنشاء مستخدم من Telegram وبقاء هويته مستقرة عبر تغيير Username» |
