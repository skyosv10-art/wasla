...
>
> **الحالة:** Baseline v1.0 · **آخر تحديث:** 2026-08-21 (Phase 04 · MR 2/6) · سابقاً 2026-08-20 (بعد دمج [MR !1](https://gitlab.com/uxxxu/wasla/-/merge_requests/1) و[MR !2](https://gitlab.com/uxxxu/wasla/-/merge_requests/2))
>
> **ملاحظة الحالة (2026-08-21، محدّث):** **W0 = 2026-08-20** (اجتازت Phase 00 Exit Gate — CI أخضر على main). **Phase 00 = Completed**، **Phase 01 = Completed** (خدمة Identity: Fastify + Drizzle/Postgres + Exit Gate E2E في CI)، و**Phase 02 = Completed** (خدمة Geography: عقود + ADR-006 + نواة مجردة + Drizzle/Postgres + Saudi seed + Fastify HTTP + وظيفة `geography-db-integration` + [بوابة خروج E2E](../12-testing/PHASE02_EXIT_GATE_E2E.md) تُشغّل identity و geography معاً في CI). **Phase 03 = Completed (2026-08-21)** — سبع مراجعات مدمجة على أساس [ADR-007](../15-decisions/ADR-007-telegram-channel-adapter-isolation-and-stack.md) (القناة طبقة توصيل لا خدمة: `packages/channel-core` محايد + `packages/telegram-adapter` وحده يعرف Bot API + `bots/*` جذور تركيب) و[ADR-008](../15-decisions/ADR-008-channel-groups-registry-and-reply-policy.md) (سجل المجموعات منفذاً عاشراً)، و**بوابة الخروج مُثبَتة لا موصوفة**: [PHASE03_EXIT_GATE_E2E.md](../12-testing/PHASE03_EXIT_GATE_E2E.md) — البوتات الثلاثة في عملية واحدة أمام خدمة هوية واحدة على HTTP، كل بوت يفتح Mini App الخاصة به، وشخص واحد = هوية واحدة، والمُهيّئ قابل للاستبدال بـ`MockChannelAdapter`. **المرحلة الحالية: Phase 04 (Customer Core) = In Progress** — مدمجتان من ستّ: MR 1/6 و**MR 2/6** (طبقة المجال النقيّة: كيانات وتحقّق وأحداث + سبعة منافذ + ثماني حالات استخدام + مُهيّئات ذاكرة و48 اختباراً تُثبت المعاينة بلا كتابة والتسليم قبل الكتابة وإعادة المحاولة على الصفّ نفسه وfail-closed وخصوصية الأحداث — [CUSTOMER_CORE_DOMAIN.md](../02-architecture/CUSTOMER_CORE_DOMAIN.md)؛ إجمالي المستودع 535 اختبار وحدة على 17 مشروعاً). أساس المرحلة MR 1/6: [ADR-009](../15-decisions/ADR-009-customer-core-placement-and-order-intake-boundary.md) يُثبّت خدمة `services/customers` مستقلّة (انحراف معلَن عن شجرة §68) وملف العميل **ملفَّ دور** بلا FK إلى الهوية و**حدّ تسليم الطلب** عبر `OrderIntakePort` وحده (الخدمة لا تكتب `orders` ولا تُولّد `order_public_id` — Phase 06 تملك آلة الحالة)، مع عقود الخدمة كاملة و[نموذج المجال](../03-domain/CUSTOMER_CORE.md) و42 اختبار حراسة. **المتبقّي: MR 3/6** (Drizzle/Postgres + `customer-db-integration` + مطابقة منافذ) · 4/6 (Fastify على 8086) · 5/6 (ربط `customer-bot`) · 6/6 (بوابة الخروج E2E مع محرّك طلبات بديل) — [HANDOFF §9](HANDOFF_NEXT_STEPS.md). وتحمل المرحلة معها عملين تشغيليين مُعلَنين من Phase 03 (مُشغّل دوري لـ`retryDueDeliveries` وناشر لصندوق الصادر — [HANDOFF §7](HANDOFF_NEXT_STEPS.md)).** Timeline أدناه نسبية إلى W0 (المُعتمد).
