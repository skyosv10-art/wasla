...
| 05 | Driver Core | W6 | ~2 أسبوعين (توازٍ مع 04) | 01,02,03 | 03,01,11,12,09 | Driver profile قابل للإدخال في Candidate pool — **قيد التنفيذ 2026-08-22 (MR 1/6 → 5/6 مدفوعة · بقيت 6/6 — 3/6: استمرارية Drizzle/Postgres بوحدة عمل واحدة تجعل العمليّة كلّها ترتكز أو تتراجع (مراجعة وثيقة واحدة = تسع كتابات في ستّة جداول)، و**بلا تغيير في أي ملفّ تحت `src/use-cases/`**، و`driver_idempotency` يُسدّ منفذاً كان بلا جدول (بعرض 8..192 لا 8..128 لأنّ المفتاح مُنطَّق)، و28 حارس انحراف مرآة + 79 اختبار تكامل في وظيفة `drivers-db-integration`، ومطابقة منافذ تُنفّذ 23 سيناريو مرّتين وتقارن الأثرين أحدهما بالآخر — [DRIVER_PERSISTENCE.md](../02-architecture/DRIVER_PERSISTENCE.md)؛ و2/6: طبقة مجال نقيّة `@wasla/drivers-service` بـ69 اختباراً بلا قاعدة وبلا HTTP، وقاعدة **«كلّ الأسباب لا أوّلها»** لأنّ الجواب قائمة عمل لسائق واحد لا تعليل رفض، و`verification_status` مُشتَقّاً بكاتب واحد، والانتهاء تاريخاً يُقارَن لا حالةً تُكتَب — [DRIVER_CORE_DOMAIN.md](../02-architecture/DRIVER_CORE_DOMAIN.md)؛ و1/6 مدفوعة: [ADR-012](../15-decisions/ADR-012-driver-core-eligibility-derivation-and-candidacy-publication.md) وعقود `services/drivers/contracts/*` وحزمة `@wasla/contracts-driver` بـ59 حارس انحراف و[DRIVER_CORE.md](../03-domain/DRIVER_CORE.md) و[CONTAINERS §4.4](../02-architecture/CONTAINERS.md)). القرار الحاكم: **الأهليّة دالّة مُشتقّة لا عمود** — تُحسب من الملفّ والمركبة والوثائق والمناطق مقابل نسخة سياسة مُقفَلة، و`unknown` fail-closed؛ والنشر إلى إسقاط الترشيح عبر `PUT /candidacy` بقيمة `driver_core` **فيُسدَّد دَين `claimed` بلا هجرة في المطابقة**؛ و`busy` يملكها التوزيع لا السائق؛ والزمن نبضة لا مؤقّت. خطة المراجعات الستّ وما بقي منها في [HANDOFF_NEXT_STEPS §13](HANDOFF_NEXT_STEPS.md) — **MR 4/6 مدفوعة** (طبقة HTTP على المنفذ 8090 — 13 مساراً و17 عمليّة، و`DriverRunner` مقبس المعاملة فلا يفتح أي معالج مسار معاملةً بنفسه؛ وثلاثة عيوب عقد صُحِّحت، و64 اختبار HTTP منها حرّاس انحراف تمنع مساراً غير موثَّق وعمليّة عقد بلا مسار — [DRIVER_HTTP.md](../04-api/DRIVER_HTTP.md)) — **MR 5/6 مدفوعة**: `HttpCandidacyPort` إلى المطابقة 8088 بفشلٍ **مصنَّف لا موحَّد** و`Idempotency-Key` لكلّ محاولة لا لكلّ محتوى، و`HttpZoneCatalogPort` إلى الجغرافيا 8081 فأُغلق الانحرافان 6 و7، و**502 تقاعَد** محروساً بحارسَين، وبوت السائق رُبِط بالمجال داخل العملية ببوّابة `DRIVER_DATABASE_URL`؛ و**عيبان فعليّان أُسقطا**: قراءةُ ترشيحٍ خارج الحارس كانت تُسقط كتابةً محليّة نجحت، و`runBot(BOT)` كان يتخطّى `buildApp()` فبوت العميل **المنشور** بلا `/places` ولا `/orders` وطاقمُه أخضر — [DRIVER_BOT_FLOWS.md](../02-architecture/DRIVER_BOT_FLOWS.md) — التالي **MR 6/6: بوابة خروج المرحلة E2E**، وهي موضع اختبار التكامل الغائب (مطابقة وجغرافيا مستمعتان + Postgres)** |
