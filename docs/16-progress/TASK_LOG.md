...
## 2026-09-09 · M5-13 · تصحيحُ ثلاثةِ عيوبٍ في العقدِ قبلَ الدمجِ (`CLM-0121`)

**السياقُ:** [PR #85](https://github.com/skyosv10-art/wasla/pull/85) كانَ **30/30 أخضرَ** وموافَقاً على دمجِه.
ومراجعةٌ آليّةٌ على الطلبِ رفعَت أربعَ ملاحظاتٍ؛ فُحصَت بالقياسِ لا بالثقةِ، فصحَّت ثلاثٌ منها.
**والدرسُ أهمُّ من الإصلاحِ:** ثلاثةُ عيوبٍ جوهريّةٍ مرَّت من سبعِ طبقاتِ تحقُّقٍ ومن 30 فحصاً
لأنّها **عيوبُ عقدٍ لا عيوبُ شيفرةٍ** — والشيفرةُ كانت متّسقةً مع عقدِها المكسورِ.

(1) **عقدٌ لا يستوفيه زوجٌ حقيقيٌّ.** فرضَ `schema.sql` صيغةَ `^WS-[0-9]{10}$` على `order_ref`
    و`store_ref` معاً. والقياسُ: `services/orders/contracts/schema.sql` يُصدرُ
    `order_public_id ~ '^ORD-[0-9]{10}$'` من متتاليةٍ في القاعدةِ، و`services/marketplace/contracts/schema.sql`
    يُعرِّفُ `store_id UUID PRIMARY KEY`. أي **لا طلبٌ ولا متجرٌ في هذه الشجرةِ نفسِها يستطيعُ
    استيفاءَ عقدِ الوفاءِ**. وهذا أسوأُ من غيابِ قيدٍ: يمرُّ خضراءَ ويسقطُ في أوّلِ صفٍّ حقيقيٍّ.
(2) **مخطَّطُ أحداثٍ يُصدِّقُ وثيقةً فارغةً.** كانَ جذرُ `events.json` خريطةَ حمولاتٍ في `properties`
    بلا `required` ولا تركيبٍ لـ`EventEnvelope`، فكانَ `{}` وأيُّ غرضٍ غريبٍ يُصدَّقُ، ولم يكن
    مستهلكٌ يستطيعُ تصديقَ حدثٍ كاملٍ أصلاً.
(3) **سطرُ تدقيقٍ لا يُجيبُ «من فعلَ هذا؟».** فاعلٌ بشريٌّ بمرجعٍ `null` كانَ يعبرُ `transition`،
    و`system` كانَ يستطيعُ حملَ مرجعِ شخصٍ فيَنسبَ إلى إنسانٍ فعلاً لم يفعلْه.

(4) **الإصلاحُ — والحدُّ في مكانِه.** صيغةُ كلِّ مرجعٍ صارت **صيغةَ مالكِه**: `ORD-` للطلبِ و`UUID`
    للمتجرِ، و`WS-` بقيَ لما نملكُه (الوفاءُ · السائقُ · الفاعلُ). و`events.json` أُعيدَ بناؤه:
    جذرٌ `oneOf` على خمسةِ أنواعٍ كاملةٍ، كلُّ نوعٍ يُركِّبُ المغلَّفَ مع حمولتِه ويُثبِّتُ
    `event_type` و`event_version` و`producer` ويرفضُ الزائدَ. وحكمُ الفاعلِ صارَ **داخلَ**
    `transition` (`DELIVERY_ACTOR_REF_INVALID`، الرمزُ التاسعُ) لا في طبقةِ HTTP — اتّساقاً مع
    قرارِ ADR-026 نفسِه: صلاحيّةٌ تعيشُ في المُتحكِّمِ تُنسى في المستهلكِ والمهمّةِ المجدولةِ.
(5) **الأثرُ في الشيفرةِ:** `services/delivery/contracts/{schema.sql,events.json,errors.md}` ·
    `services/delivery/src/domain/{model.ts,state-machine.ts}` ·
    `packages/contracts/delivery/src/index.ts` (`DELIVERY_REF_PATTERNS`) · ADR-026 §3.
(6) **المقيسُ من تشغيلٍ فعليٍّ بعدَ التصحيحِ:** وحدةُ الخدمةِ **21/21** (كانت 17) ·
    عقودُ الحزمةِ **10/10** (كانت 4) · `pnpm -r typecheck` أخضرُ ·
    `pnpm -r test` **3931 اختباراً في 239 ملفّاً** (كانت 3921/238) · صفرُ إخفاقٍ.
(7) **حدٌّ مُعلَنٌ لا يُطوى:** اختبارُ عقدِ الأحداثِ يفحصُ **بنيةَ المخطَّطِ المانعةَ** ولا يُشغِّلُ
    مدقِّقَ JSON Schema على مثالٍ حيٍّ، لأنّه لا مدقّقَ معلَنٌ اعتماداً في هذه الشجرةِ
    (`ajv` حاضرٌ عرَضاً لا إعلاناً)، وإضافةُ اعتمادٍ قرارٌ يمرُّ ببوّابةِ `M0-06` لا يُتَّخذُ
    ضمناً في مراجعةٍ تأسيسيّةٍ. **يُنقَلُ للمراجعةِ التاليةِ.**
(8) **وما لم يُتحقَّق منه:** الملاحظةُ الرابعةُ من المراجعةِ الآليّةِ **لم تُعرَض** (حجبَتها إعداداتُ
    المستودعِ) فلم تُفحَص ولا يُدَّعى ردُّها · ولا محوّلَ PostgreSQL ولا طبقةَ HTTP ولا مستهلكَ
    أحداثٍ بعدُ · وبوّابةُ `inventory/payment E2E` لا تُدَّعى ولا يجوزُ ادّعاؤها قبلَ `M5-17`
    وحجزِ المخزونِ · **والحالةُ تبقى `In Progress`** (§9).
