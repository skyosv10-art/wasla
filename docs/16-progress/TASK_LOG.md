...
**Task:** تحويل نصّ بوابة الخروج المنشور («كل Bot يفتح Mini App المناسبة، ويمكن استبدال Telegram adapter في الاختبارات بـMock Adapter») من جملة في خريطة الطريق إلى **اختبار يفشل إذا كُسر**: البوتات الثلاثة معاً في عملية واحدة، أمام خدمة هوية واحدة تستمع فعلياً على HTTP، ومخازن قناة مشتركة — ثم إغلاق المرحلة. **Status:** Completed (446 اختباراً تنجح محلياً: 445 بمخازن الذاكرة + اختبار صفوف يعمل مع `DATABASE_URL`؛ الجديد: 8) · **MR:** [!MR7](https://gitlab.com/uxxxu/wasla/-/merge_requests/MR7) · **ADR:** لا حاجة (انظر السؤال 8)
