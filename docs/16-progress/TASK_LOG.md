...
### [2026-09-02] M1-03 — عائقُ ثمانيةِ خطوطٍ ارتفعَ: البوّابةُ `PASSED` والحالةُ `GATED`
- **Files:** `docs/12-testing/M1-03_GATE.md` · `docs/12-testing/ci-evidence/2026-09-02T031500Z-m1-03-ci-gate/{verdict.json,README.md}` · `docs/16-progress/{WORK_CLAIMS,LAUNCH_EXECUTION_BOARD,TASK_LOG}.md`
- **Services:** —
- **Work Item(s):** M1-03
- **Why:** بوّابةُ `M1-03` كانت **14 ✅ · بندٌ مانعٌ واحدٌ** هو الثالثَ عشرَ (دليلُ CI): `NOT VERIFIED · REASON = Job did not start · BLOCKING = Yes`، عبرَ **ثمانيةِ خطوطِ GitLab متتاليةٍ** بـ`jobs_started: 0` و`ci_quota_exceeded`. وبعدَ `M0-21` (نقلُ الخطِّ إلى GitHub Actions) و`M0-22B` (نقلُ العشرينَ وظيفةً) **صارَ الدليلُ متاحاً**، فبقاءُ البندِ ⚪ يصيرُ خطأً في الوصفِ لا احتياطاً.
- **What:** (1) دليلُ CI جديدٌ للتشغيلِ [`33569807797`](https://github.com/skyosv10-art/wasla/actions/runs/33569807797) على `main` (`push` · `0a752470`): **26 وظيفةً · 26 `started_at ≠ null` · 26 `success`**، بلا تكرارِ ملفٍّ خامٍ (يُحالُ إلى مجلَّدِ `M0-22B`) عملاً بقاعدةِ الدليلِ الكانونيِّ §2.0.1 — **وهي نفسُها القاعدةُ التي تُوجِبُ مجلَّداً جديداً حينَ تبدأُ وظيفةٌ فعلاً**. (2) البندُ 13 → ✅، والطبقةُ الوسطى `CI ⚪ NOT VERIFIED` → **`✅ VERIFIED`**، والمحصّلةُ **15/15** والبوّابةُ `PASSED`. (3) §2.2 جديدةٌ تُقابِلُ ما كانَ بما صارَ. (4) تحريرُ `CLM-0029`/`CLM-0030` وحجزُ `CLM-0036`.
- **Tests:** `pnpm --filter @wasla/service-auth test` = **7 ملفّاتٍ · 127 اختباراً · 127 ناجحاً** على الشجرةِ نفسِها · `verify-governance.sh` · `scripts/verify.sh`. **ودليلُ CI الخاصُّ بالعنصرِ لا انتسابٌ عامٌّ:** `service-identity-enforcement.e2e.test.ts` — **8 اختباراتٍ ناجحةٍ** داخلَ `exit-gate-e2e (dispatch)` بقاعدةِ بياناتٍ حقيقيّةٍ، من سجلٍّ خامٍ محفوظٍ، ونقلُه مشروعٌ لأنّ `git diff --stat 9afd9d5 0a75247` = **صفرُ ملفّاتٍ**.
- **Limits declared:** سجلّاتُ التشغيلِ `33569807797` الخامُ **غيرُ محفوظةٍ** — لا لأنّها لم تُطلَب بل لأنّ مضيفَ التنزيلِ (`productionresultssa9.blob.core.windows.net`) محجوبٌ في بيئةِ التنفيذِ (`403`)؛ فحزمةُ `service-auth` مُثبَتةٌ **على مستوى الوظيفةِ** (`pnpm -r run test` = `success`) لا على مستوى السطرِ. و`PRODUCTION` ما زالت `NOT VERIFIED`. والخطُّ **يُنفِّذُ ولا يَمنعُ** (`required_status_checks` غيرُ مُسجَّلٍ · `M0-22A` محجوبٌ · `RISK-0002` مفتوحٌ).
- **Status not claimed:** عمودُ اللوحةِ يبقى `Ready for Gate` **لأنّ `GATED` ليست من حالاتِ اللوحةِ المسموحةِ**، لا لأنّ البوّابةَ لم تمرَّ — وقد نُصَّ ذلكَ في الخليّةِ. **و`COMPLETED` سلطةُ مالكِ البرنامجِ وحدَه** (البروتوكول §9)، ولم تُكتَب.
- **Stacked MR — يُعلَنُ ولا يُطوى:** هدفُ الطلبِ **`chore/m0-22b-gate-closeout` لا `main`**، وذلكَ ضرورةٌ لا اختصارٌ: دليلُ هذا العنصرِ يُحيلُ إلى الملفّاتِ الخامِ في `ci-evidence/2026-09-02T023000Z-m0-22b-main-run/` وهي **لم تُدمَج في `main` بعدُ** (طلبُ `M0-22B` مفتوحٌ · بوّابةُ مالكٍ)، فاستهدافُ `main` يكسرُ الإحالةَ أو يُوجِبُ نسخَ الخامِ نسخاً ثانياً — وكلاهما مرفوضٌ. والتكديسُ **مشروعٌ بنصِّ الفحصِ** [`validate-mr-target.sh`](../../scripts/checks/validate-mr-target.sh) («هدفٌ متقدِّمٌ بالتزامٍ أو أكثرَ → يمرُّ بتنبيهٍ»)، **وحدُّه المُعلَنُ**: لو دُمج `M0-22B` إلى `main` بعدَ دمجِ هذا الطلبِ إليه لا قبلَه، بقي هذا العملُ خارجَ `main` — عينُ عيبِ `M0-16`/`MR !97`. **فترتيبُ الدمجِ قيدٌ على المالكِ: `M0-22B` أوّلاً.**
- **Governance base:** فُحصت الحوكمةُ بالأساسِ الصحيحِ `verify-governance.sh chore/m0-22b-gate-closeout HEAD` = **12 ناجحاً · تخطٍّ واحدٌ مُعلَنٌ** (الفحصُ 8: الدمجُ غيرُ مانعٍ). **ولم يُعطَّل فحصٌ ولا وُسِّع نطاقُ حجزٍ لإسكاتِه**: قياسُه بأساسِ `origin/main` كانَ يُظهِرُ `RISK_REGISTER.md` خارجَ نطاقِ `CLM-0036`، وهو ملفُّ `M0-22B` الوارثُ من الفرعِ الأمِّ لا ملفُّ هذا العملِ.
- **تصحيحٌ ذاتيٌّ في اليومِ نفسِه — أضعفتُ الدليلَ ثمّ قوّاه القياسُ:** كُتبَ أوّلاً أنّ `packages/service-auth` مُثبَتةٌ **على مستوى الوظيفةِ لا السطرِ**؛ ثمّ فُتِّشَ أرتفاكتُ `test-log` المحفوظُ في المستودعِ فوُجدَ فيه **`7 ملفّاتٍ · 127/127` سطراً سطراً** للتشغيلِ [`33498726052`](https://github.com/skyosv10-art/wasla/actions/runs/33498726052) (`push` · `main` · `fce1f054`). **والجسرُ إلى `main` اليومَ قياسٌ لا ظنٌّ:** `git diff --name-only fce1f054 origin/main -- packages services` = **صفرُ ملفّاتٍ**، و`git diff … -- packages/service-auth` = **صفرُ بايتاتٍ**؛ فالفرقُ بينَ التشغيلَينِ ملفّا CI ووثائقُ لا شفرةٌ. **والخطأُ يُصحَّحُ ولا يُمحى**: نصُّ الحدِّ الأوّلِ باقٍ مع تصحيحِه في مكانِه. **ودرسُه:** «الدليلُ غيرُ متاحٍ» لا تُقالُ قبلَ تفتيشِ ما هو **مُلتزَمٌ في المستودعِ أصلاً** — كانَ الأرتفاكتُ على مبعدةِ أمرٍ واحدٍ.
- **CI لهذا الفرعِ: `NOT VERIFIED` — والسببُ مقيسٌ لا مظنونٌ.** أُنشئ [PR #7](https://github.com/skyosv10-art/wasla/pull/7) ثمّ سُئلَ المستودعُ عن تشغيلٍ لهذا الفرعِ فأجابَ **صفراً**: `.github/workflows/ci.yml` مشروطٌ بـ`pull_request: branches: [main]` و`push: branches: [main]`، **فالطلبُ المكدَّسُ لا يُطلِقُ خطّاً بحكمِ التصميمِ**. وهذا **«لم يُطلَق»** لا «أخفَق» ولا «الشفرةُ معيبةٌ» — والفرقُ واجبٌ.
- **ولمَ لم يُلجَأ إلى `workflow_dispatch` ولا إلى توسيعِ المُطلِقاتِ:** الأوّلُ يُشغِّلُ الخطَّ بلا `GITHUB_BASE_REF`، فتقيسُ وظيفةُ `governance-guard` بأساسِ `main` وتَرفُضُ رفضاً صحيحاً في نفسِها لأنّ `RISK_REGISTER.md` — **ملفُّ الفرعِ الأمِّ الموروثُ** — خارجُ نطاقِ `CLM-0036`؛ فتُسجَّلُ حمرةٌ سببُها **خطأُ أساسِ القياسِ لا خطأُ الشفرةِ**، وتسجيلُ حمرةٍ كاذبةٍ كتسجيلِ خضرةٍ كاذبةٍ. والثاني (`branches: [main, 'chore/**']`) **خارجُ نطاقِ الحجزِ** ويُصادِمُ قاعدةَ المستودعِ: لا يُوسَّعُ حارسٌ في الدفعةِ التي يُقاسُ بها. **ولم يُوسَّع نطاقُ `CLM-0036` ليشملَ `docs/07-security/`** إذ يمنعُه [`WORK_CLAIM_RULE.md` §6](../00-rules/WORK_CLAIM_RULE.md) (لا توسيعَ بعدَ بدءِ الكتابةِ)، **والصوابُ أنّ العيبَ في أساسِ القياسِ لا في السجلِّ**: الأساسُ الصحيحُ لطلبٍ مكدَّسٍ هو هدفُه، وبه الحوكمةُ خضراءُ (12 ناجحاً).
- **وما يُثبِتُه هذا الفرعُ بلا خطٍّ، وما لا يُثبِتُه:** تغييرُه **وثائقُ محضٌ** (`git diff --name-only chore/m0-22b-gate-closeout HEAD` = **ستةُ ملفّاتٍ كلُّها تحتَ `docs/`** · صفرُ ملفِّ شفرةٍ · والأساسُ هنا هدفُ الطلبِ لا `main`، فالاثنا عشرَ ملفاً في قياسِ `origin/main` تضمُّ ستّةَ ملفّاتِ الفرعِ الأمِّ الموروثةَ)، فلا وظيفةَ بناءٍ أو نوعٍ أو اختبارٍ يمسُّها منطقُه. **ولا يُقرَأُ هذا إذناً بتخطّي الخطِّ:** يُشغَّلُ الخطُّ على هذا العملِ حينَ يُدمَجُ `PR #6` إلى `main` ويُعادُ توجيهُ هذا الطلبِ إليه، **وذلكَ بعدَ بوّابةِ المالكِ لا قبلَها**.
- **Next:** طلبُ دمجٍ مكدَّسٌ من `chore/m1-03-gate-ci-evidence` وقراءةُ نتيجةِ الخطِّ الفعليّةِ، **دونَ دمجٍ** (بوّابةُ مالكٍ · §13). ثمّ بقيّةُ عناصرِ `Ready for Gate` التي لا وثيقةَ بوّابةٍ لها.
