...
- **Work Item(s):** M0-01 · M0-03
- **Claim:** `CLM-0037` (M0-01) · `CLM-0038` (M0-03) — نطاقُهما `docs/12-testing/` وحدَه
- **Files:** `docs/12-testing/M0-01_GATE.md` (جديدٌ) · `docs/12-testing/M0-03_GATE.md` (جديدٌ) · `docs/12-testing/ci-evidence/2026-09-02T040000Z-m0-phase0-gates/` (جديدٌ: `README.md` · `verdict.json` · `jobs-*.json` ×4 · `local-measurements/m0-03-race-reproduced.raw.txt`) · `docs/16-progress/{WORK_CLAIMS,LAUNCH_EXECUTION_BOARD,TASK_LOG}.md`
- **Services:** — (لا سطرَ شفرةٍ: `git diff --name-only origin/main HEAD -- packages services scripts` = **صفرٌ**)
- **Problem:** عشرونَ عنصراً في اللوحةِ `Ready for Gate` **بوثيقتَي بوّابةٍ لا أكثرَ** في المستودعِ كلِّه. والحالةُ لا تُرفَعُ بلا وثيقةٍ تجمعُ الأدلّةَ الثلاثةَ التي يطلبها [`STATUS_MODEL.md` §2.8](../00-rules/STATUS_MODEL.md) في موضعٍ **يُدقَّقُ** — فبقيَت الأدلّةُ مبثوثةً في سجلِّ مهامٍّ يبلغُ آلافَ الأسطرِ، وهذا ليس دليلاً بل ذِكرى دليلٍ.
- **Root Cause:** المانعُ الأصليُّ **لم يكن شكّاً في الشفرةِ** بل **حصّةَ CI المنتهيةَ**: مكتوبٌ في هذا السجلِّ بتاريخِ 2026-08-27 أنّ `db-integration` و`governance-guard` **لم تُشغَّلا** على عملِ `M0-03`، وأنّ الأرقامَ كلَّها محلّيّةٌ، **ومن ثمّ `Ready for Gate` لا `Completed`**. وقد زالَ هذا المانعُ بإصلاحِ `M0-21`/`M0-22B` ولم يُقطَف أثرُه بعدُ.
- **Fix:** بوّابتانِ مكتوبتانِ بقالبِ APPENDIX B، كلٌّ منهما بثلاثِ طبقاتٍ (`LOCAL` · `CI` · `PRODUCTION`) وجدولِ بنودٍ كلُّ بندٍ فيه **مسارُ دليلٍ يُفتَحُ** لا وصفٌ.
- **Verification — أربعُ تنفيذاتٍ لا قراءةٌ واحدةٌ أربعَ مرّاتٍ:** التشغيلاتُ [`33566652310`](https://github.com/skyosv10-art/wasla/actions/runs/33566652310) · [`33567256847`](https://github.com/skyosv10-art/wasla/actions/runs/33567256847) · [`33569807797`](https://github.com/skyosv10-art/wasla/actions/runs/33569807797) · [`33573216083`](https://github.com/skyosv10-art/wasla/actions/runs/33573216083) — **26/26 وظيفةً بدأت ونجحت في كلٍّ منها**، وصفرُ وظيفةٍ بـ`started_at = null`. **وجَسرُها إلى `main` قياسٌ:** `git diff --name-only <sha> 0a752470 -- packages services scripts` = **صفرُ ملفّاتٍ** في الأربعةِ، فالفرقُ وثائقُ وملفّا خطٍّ لا شفرةٌ.
- **Verification — طفرتانِ حقيقيّتانِ أُجريتا اليومَ لا استُشهِدَ بهما:**
  1. **`M0-01`:** حُذفَ `uuidIdGenerator` من `new SubscriptionService(...)` في `services/subscriptions/src/http/server.ts` فأجابَ التشغيلُ `composition.test.ts (10 tests | 2 failed)` · `Tests 2 failed | 213 passed (215)`؛ ثمّ رُدَّ الملفُّ وقِيسَ `git diff --stat services/` = **فارغٌ**.
  2. **`M0-03`:** شُغِّلَ ملفّا التكاملِ **عمليّتَينِ متوازيتَينِ** على قاعدةٍ واحدةٍ ثلاثَ مرّاتٍ، فسقطت الثانيةُ بـ`error: relation "identity_links" does not exist`؛ وبالتسلسلِ **10 من 10 نجاحاً**. فالفرقُ الوحيدُ التوازي.
- **⚠️ حدٌّ يُعلَنُ ولا يُطوى — طفرةٌ لم تُعِد إنتاجَ سقوطٍ، والسببُ مقيسٌ:** `--fileParallelism --poolOptions.forks.maxForks=4` أجابَ `5 passed` **ثلاثاً من ثلاثٍ**. و`nproc` في بيئةِ التنفيذِ = **2**، وهو عينُ ما حذّرَ منه إدخالُ 2026-08-27: على آلةٍ بنواتَين يجعلُ vitest حدَّ العواملِ 1 فيتسلسلُ الملفّانِ **عرَضاً** وتظهرُ الحزمةُ **خضراءَ كاذبةً**. **فهذه الخُضرةُ سُجِّلت مثالاً على العيبِ لا دليلاً على السلامةِ**، ولذلك أُعيدَ إنتاجُ السباقِ بمسارٍ لا يتأثّرُ بعددِ النوى.
- **بنيةٌ تحتيّةٌ استُحدثت لا افتُرضت:** لم تكن في البيئةِ قاعدةٌ (`psql` و`docker` غائبانِ) فكانَ كلُّ اختبارِ تكاملٍ **غيرَ قابلٍ للتحقُّقِ محلّيّاً**. فنُصِّبت **PostgreSQL 18.6** وأُنشئت الثمانيَ عشرةَ قاعدةً التي تسمّيها مصفوفتا CI بأسمائها. **وإصدارُ CI هو `postgres:15` لا 18**، والفرقُ يُعلَنُ ولا يُدَّعى تطابقُ سلوكِ DDL بينَهما في كلِّ حالٍ.
- **ولمَ لم يُدخَل `M0-02` في الدفعةِ معَ أنّ دليلَه جاهزٌ:** `CLM-0004` نشطٌ عليه **لمالكٍ بشريٍّ** وفرعُه قائمٌ وعملُه غيرُ مدموجٍ. فحجزٌ ثانٍ يرفضُه فحصُ التكرارِ بحقٍّ، والبدءُ بلا حجزٍ يخالفُ القاعدةَ. **فالعنصرُ يُترَكُ لمالكِه**، ودليلُه (`db-integration (drivers)` أخضرُ أربعَ مرّاتٍ · `port-conformance.integration` 23 حالةً) **محفوظٌ في مجلَّدِ هذه الدفعةِ ليجدَه جاهزاً** لا ليُبنى عليه حكمٌ من غيرِ مالكِه.
- **ما لم يُكتَب:** لا `Completed` لعنصرٍ (سلطةُ مالكِ البرنامجِ · البروتوكول §9) · ولا `GATED` في عمودِ اللوحةِ (ليست من حالاتِها المسموحةِ) · ولا دمجٌ.
- **Next:** بوّاباتُ باقي عناصرِ `Ready for Gate` بالترتيبِ نفسِه ومعَ القاعدةِ المحلّيّةِ الجديدةِ التي تُتيحُ قياسَ التكاملِ فعلاً.
- **Related:** `CLM-0037` · `CLM-0038` · [`M0-01_GATE.md`](../12-testing/M0-01_GATE.md) · [`M0-03_GATE.md`](../12-testing/M0-03_GATE.md) · `RISK-0002` (الخطُّ يُنفِّذُ ولا يَمنعُ) · `RISK-0014` (`M0-18`)
