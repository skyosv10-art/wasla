...
- **Tests:** **مقيسٌ لا متوقَّعٌ.** (1) `packages/telegram-adapter`: `typecheck` رمزُ 0 · **9 ملفّاتٍ · 132 حالةً ناجحةً · 0 فاشلةٌ** (منها `init-data.test.ts` **33**). (2) `services/identity`: `typecheck` رمزُ 0 · **5 ملفّاتٍ · 52 ناجحةً** (منها `session.test.ts` **28**) · واختباراتُ التكاملِ على Postgres حقيقيٍّ **5/5** بعدَ إضافةِ `identity_sessions` إلى قوائمِ `DROP TABLE`. (3) **بوابةُ M1-02 من طرفٍ إلى طرفٍ** في `packages/channel-e2e`: **17 حالةً · 0 فاشلةٌ على Postgres 17 حقيقيٍّ** (وعلى منفذِ الذاكرةِ 7 ناجحةٌ و**10 متخطّاةٌ بإعلانٍ** لأنّ منعَ الإعادةَ والقيودَ لا يُثبِتها إلّا محرّكٌ). (4) **`scripts/verify.sh` كاملةً بالمرجعِ المُكدَّسِ الصحيحِ: 4 نجحَ · 2 أخفقَ** — الأنواعُ رمزُ 0، والاختبارُ **212 ملفاً · 3600 ناجحاً · 0 فاشلٌ · 31 متخطّىً بإعلانٍ**، وحزمةُ إثباتِ الحوكمةِ **142 ناجحاً · 1 فاشلٌ**، والفاشلُ **واحدٌ في جوهرِه**: البابُ الرابعُ من الفحصِ الحادي عشرَ (شجرةٌ مُعدَّلةٌ بلا `dirty_reason`) — أثرُ توليدِ الأساسِ قبلَ الالتزامِ، لا انحدارٌ في كودٍ. **ويُعلَن ما لم يُتحقَّق منه:** لا CI، ولا قياسَ أداءٍ، ولا تنظيفَ دوريّاً للجلساتِ المنتهيةِ (دَينٌ مُعلَنٌ في ADR-019 §3).
