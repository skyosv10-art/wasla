...
| `services/search/` | In Progress | M5-12 (مراجعةٌ تأسيسيّةٌ 1/N · بدأت 2026-09-08) | Phase 12 | **[ADR-025](../15-decisions/ADR-025-marketplace-search-read-model.md):** البحثُ نموذجُ قراءةٍ مشتقٌّ من أحداثِ السوقِ لا قراءةٌ مباشرةٌ. أُنشئت: عقودُ `contracts/` (`schema.sql` بفهارسِ `tsvector`+`pg_trgm` · `api.openapi.yml` · `events.json` · `errors.md`)، ونواةُ نطاقِ `src/domain/` (`query`/`ranking`/`visibility`/`model`) — 24 وحدةً + 9 عقودٍ خضراءَ. **يُؤجَّلُ:** المستهلكُ (relay) · بوّابةُ relevance/load · HTTP (Fastify) · تكاملٌ ووظائفُ CI. |
