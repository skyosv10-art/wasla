 ...
 * - **جِذرُها لا يعرفُ إطارَ ويبٍ، والوسيطُ المركزيُّ صارَ موجوداً (`M1-04`).**
 *   كان النصُّ هنا: «لا تعرف إطارَ ويبٍ … ورَبطُه بـFastify في طبقةِ HTTP عندَ
 *   كلِّ خدمةٍ مفروضةٍ حتّى يُوحِّده الوسيطُ المركزيُّ (`M1-04`)». فالحدُّ كان
 *   مؤقَّتاً بنصِّه، وقد وُحِّدَ: [`./fastify.ts`](./fastify.ts) — تصديرٌ فرعيٌّ
 *   `@wasla/service-auth/fastify` **لا يُصدَّرُ من هذا الملفِّ بقصدٍ**.
 *   و`fastify` فيها `peerDependency` اختياريٌّ لا `dependency`، فمن لا يستوردُ
 *   التصديرَ الفرعيَّ لا يحملُها، ويبقى القلبُ (`enforce` · `token` · `keys` ·
 *   `replay`) جاهلاً بـFastify. **والفرضُ نفسُه ما زالَ قراراً يُرَدُّ** لا رَدّاً
 *   يُرسَل: الوسيطُ يربطُ القرارَ بالإطارِ ولا يتّخذُه.
