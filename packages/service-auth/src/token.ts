 ...
 *
 *     wsvc3.<base64url(JSON الحِمْل)>.<base64url(HMAC-SHA256)>
 *
 ...
 * **ثابتةٌ في الكودِ**، والنسخةُ وحدَها في البادئةِ `wsvc3`، وأيُّ بادئةٍ أخرى
 * تُرفَض قبلَ أن يُفَكَّ ترميزُ حرفٍ واحد.
 *
 * والتوقيعُ يُحسَب على **النصِّ المُرسَلِ حرفيّاً** (`wsvc3.<payload>`) لا على
 ...
 * كلُّ رمزٍ مربوطٌ بـ`<METHOD> <path>[?<query مُرتَّبٍ>]` واحدٍ. فالرمزُ المُلتقَطُ من
 * نداءِ قراءةٍ لا يُعاد استخدامُه على نداءِ حذفٍ. ولا يُقبَل رمزٌ بلا ربطٍ: خيارُ
 * «بلا ربط» كان سيُنتِج حاملَ سلطةٍ عامّاً بعمرِ دقيقةٍ، وهذا ما كان يُفترَض منعُه.
 *
 * [إضافةٌ 2026-09-17 · `ADR-036`] وكانَ الربطُ **بلا سلسلةِ استعلامٍ** (`ADR-021 §4`)،
 * فرمزُ `GET /orders/lookup?order_public_id=A` كانَ صالحاً حرفيّاً لـ`…=B`. وهذا هوَ
 * `RISK-0026`، وقد أُقفِلَ بضمِّ الاستعلامِ **مُرتَّباً مُعجَميّاً** — لا حرفيّاً — كي
 * يزولَ الخطرُ ويبقى سببُ الاستثناءِ الأصليُّ (إعادةُ ترتيبِ الوسطاءِ) مُعالَجاً.
 *
 ...
 *
 * ── ولِمَ `wsvc3` (2026-09-17 · `ADR-036`) ──────────────────────────────────
 * تغيَّرَ **معنى** `req` لا شكلُه: صارَ يضمُّ سلسلةَ الاستعلامِ مُرتَّبةً. ومعنىً
 * يتغيَّرُ بلا بادئةٍ تتغيَّرُ أخطرُ من حقلٍ يُضاف: مُوقِّعٌ قديمٌ ومُتحقِّقٌ جديدٌ
 * كانا سيتوافقانِ على المساراتِ بلا استعلامٍ ويختلفانِ على غيرِها — أي عطلٌ
 * **جزئيٌّ صامتٌ** يُقرأُ خطأَ شبكةٍ لا خطأَ نسخةٍ. فبُدِّلت البادئةُ كي يكونَ
 * الرفضُ `unsupported_scheme` صريحاً في أوّلِ محرفٍ. **وشرطُ النشرِ مُعلَنٌ:**
 * المُوقِّعُ والمُتحقِّقُ يُنشَرانِ معاً — ولا يُدَّعى قياسُ ذلكَ اليومَ إذ لا نشرَ
 * إنتاجيّاً مقيساً في هذا المستودعِ.
 *
...
export const SERVICE_TOKEN_SCHEME = "wsvc3";

/**
 * الصيغُ السابقةُ. تُذكَر هنا **لتُرفَض بأسمائِها** لا لتُقبَل: بقاءُ واحدةٍ منها
 * في القراءةِ يُبطِل ما بُدِّلت الصيغةُ لأجلِه.
 *
 * - `wsvc1`: بلا `jti` فبلا حارسِ إعادةٍ.
 * - `wsvc2`: ربطٌ بـ`<METHOD> <path>` **بلا سلسلةِ استعلامٍ** (`RISK-0026`) — فرمزٌ
 *   وُقِّعَ لقراءةِ `ORD-0000000001` كانَ صالحاً حرفيّاً لقراءةِ `ORD-0000009999`.
 *   وقَبولُه اليومَ يُبقي البابَ الذي فُتِحَت `wsvc3` لإغلاقِه.
 */
export const SUPERSEDED_TOKEN_SCHEMES: readonly string[] = ["wsvc1", "wsvc2"];
...
/**
 * يُطبِّع سلسلةَ الاستعلامِ تطبيعاً **لا يعتمد على ترتيبِ الوسيطِ**: تُفكَّك
 * الأزواجُ، ثمَّ تُرتَّب بالاسمِ ثمَّ بالقيمةِ، ثمَّ يُعاد ترميزُها ترميزاً واحداً.
 *
 * **ولِمَ الترتيبُ لا القبولُ الحرفيُّ:** `ADR-021 §4` استثنى الاستعلامَ من الربطِ
 * **لسببٍ صحيحٍ في وقتِه**: الوسطاءُ والوكلاءُ يُعيدونَ ترتيبَ المعاملاتِ، فتوقيعٌ
 * على النصِّ الحرفيِّ يُكسَر بإعادةِ ترتيبٍ بريئةٍ. والترتيبُ المُعجَميُّ يُزيل هذا
 * السببَ نفسَه: `?b=2&a=1` و`?a=1&b=2` يُنتِجانِ الربطَ عينَه، بينما `order_public_id`
 * مختلفٌ يُنتِج ربطاً مختلفاً. **فالاستثناءُ لم يُلغَ بالقرارِ بل زالَ سببُه بالقياسِ.**
 *
 * **وما لا يُطبَّع عمداً:** المفتاحُ المُكرَّر يبقى مُكرَّراً (`?id=1&id=2` ليس `?id=1`)
 * لأنَّ ابتلاعَ التكرارِ يجعلُ ربطَينِ مختلفَينِ واحداً — وهو بابُ الالتفافِ نفسُه.
 * والقيمةُ الفارغةُ تبقى مُعلَنةً (`?flag=`) ولا تُساوي غيابَ المفتاح.
 */
function canonicalQuery(rawQuery: string): string {
  if (rawQuery === "") return "";
  const params = new URLSearchParams(rawQuery);
  const pairs: Array<readonly [string, string]> = [];
  for (const [name, value] of params) pairs.push([name, value] as const);
  ...
  for (const [name, value] of pairs) encoded.append(name, value);
  return encoded.toString();
}

/**
 * يُطبِّع الربطَ: الطريقةُ كبيرةٌ، والمسارُ بلا شرطةٍ أخيرةٍ، **وسلسلةُ الاستعلامِ
 * مضمومةٌ بترتيبٍ مُعجَميٍّ** (`RISK-0026`).
 *
 * **ولِمَ ضُمَّ الاستعلامُ:** أوّلُ موضعٍ ماديٍّ كانَ `GET /orders/lookup?order_public_id=…`
 * — مسارٌ يحملُ هويّةَ المَورِدِ في الاستعلامِ لا في المسارِ. **وأُعيدَ القياسُ**
 * فوُجِدَ أكثرُ من موضعٍ يقرأُ مُرشِّحاً من الاستعلامِ، فالعلاجُ في **الآلةِ** لا في
 * مسارٍ واحدٍ: ما دامَ الربطُ يشملُ الاستعلامَ، فكلُّ مسارٍ يقرأُ منه محميٌّ بالبناءِ
 * لا بيقظةِ مراجعٍ — ولا يحتاجُ المسارُ القادمُ أن يُتذكَّر.
 *
 * **وما لا يُدَّعى:** هذا يُقيِّد الرمزَ بالطلبِ الذي وُقِّعَ لأجلِه، ولا يمنعُ مَن
 * يملكُ المفتاحَ من توقيعِ أيِّ طلبٍ شاءَ. وهو ربطُ نيّةٍ لا تفويضُ مَورِدٍ.
 */
export function canonicalRequestBinding(method: string, path: string): string {
  const upperMethod = method.trim().toUpperCase();
  const separator = path.indexOf("?");
  const rawPath = separator < 0 ? path : path.slice(0, separator);
  ...
  return query === ""
    ? `${upperMethod} ${normalized}`
    : `${upperMethod} ${normalized}?${query}`;
}
