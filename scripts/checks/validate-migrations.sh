...
for svc in "${ENROLLED[@]:-}"; do
  ...
  #
  # **العطبُ المقيسُ في البابِ نفسِهِ (M0-34 · 2026-09-14):** كانَ 4-ب يقبلُ
  # **أيَّ** ملفٍّ تحتَ `src/` يحملُ العلامةَ — حتّى ملفّاً لا شيءَ فيهِ غيرُ
  # تعليقٍ. وحالتا القبولِ في حزمةِ الحوكمةِ كانتا تكتبانِ ملفَّ تعليقٍ محضاً
  # وتمرّانِ خضراءَ: أي أنَّ الحارسَ كانَ يقيسُ **وجودَ سطرٍ** لا **وجودَ برهانٍ**،
  # وهذا هوَ عينُ ما بُنيَ لمنعِهِ. فالعلامةُ صارت تُلزِمُ حاملَها بخمسٍ:
  #   1) ملفُّ اختبارٍ يُنفِّذُهُ مُشغِّلُ الحزمةِ (`*.test.ts` تحتَ `src/`)،
  #   2) فيهِ دعوى تشغيلٍ واحدةٌ على الأقلِّ (`it(` أو `test(`)،
  #   3) غيرُ مُتخطًّى بلا شرطٍ (`describe.skip` · `it.skip` · `.todo`)
  #      — و`skipIf` مشروعٌ: تخطٍّ بشرطِ بيئةٍ مقيسٍ لا تخطٍّ أبديٌّ،
  #   4) يزرعُ صفوفاً فعلاً (`INSERT INTO`) — فالمأهولةُ هيَ الدعوى كلُّها،
  #   5) يُطبِّقُ الترحيلاتِ من مصدرِها (`_journal.json`) لا SQL مكتوباً بيدٍ
  #      في الاختبارِ — وإلّا برهنَ على نصٍّ لا على ترحيلٍ.
  #
  # **وحدُّ هذا البابِ مُعلَنٌ:** الخمسُ ساكنةٌ تُقاسُ على النصِّ، فلا تُثبِتُ
  # **ترتيبَ** الزرعِ قبلَ الترقيةِ ولا صدقَ التوكيداتِ — ذاكَ تُثبِتُهُ البوّابةُ
  # حينَ تركضُ. وما تمنعُهُ: أن يُشترى الأخضرُ بتعليقٍ أو بتخطٍّ أبديٍّ.
  covered_all=0
  declared_tags=""
  while IFS= read -r pf; do
    ...
    rel="${pf#"$svc/"}"
    if [[ "$pf" != *.test.ts ]]; then
      bad "$svc/$rel: علامةُ برهانِ الترقيةِ في ملفٍّ لا يُنفِّذُهُ مُشغِّلُ الاختباراتِ (لا ينتهي بـ«.test.ts») — علامةٌ لا تركضُ ليست برهاناً (M0-34)."
      continue
    fi
    if ! grep -qE '(^|[^[:alnum:]_])(it|test)[[:space:]]*(\.[a-zA-Z]+)?[[:space:]]*\(' "$pf"; then
      bad "$svc/$rel: البرهانُ بلا دعوى تشغيلٍ واحدةٍ («it(» أو «test(») — تعليقٌ يشرحُ برهاناً ليسَ برهاناً."
      continue
    fi
    if grep -qE '(describe|it|test)[[:space:]]*\.[[:space:]]*(skip|todo)[[:space:]]*\(' "$pf"; then
      bad "$svc/$rel: البرهانُ مُتخطًّى بلا شرطٍ («.skip(» أو «.todo(») — والتخطّي المشروعُ «skipIf» بشرطِ بيئةٍ مقيسٍ (M0-34)."
      continue
    fi
    if ! grep -qiE 'INSERT[[:space:]]+INTO' "$pf"; then
      bad "$svc/$rel: البرهانُ لا يزرعُ صفّاً واحداً («INSERT INTO» غائبٌ) — والقاعدةُ المأهولةُ هيَ الدعوى كلُّها، وقاعدةٌ فارغةٌ تُخفي 23502."
      continue
    fi
    if ! grep -q '_journal.json' "$pf"; then
      bad "$svc/$rel: البرهانُ لا يُطبِّقُ الترحيلاتِ من مصدرِها («_journal.json» غيرُ مقروءٍ) — برهانٌ على SQL مكتوبٍ في الاختبارِ لا على الترحيلِ المنشورِ."
      continue
    fi
