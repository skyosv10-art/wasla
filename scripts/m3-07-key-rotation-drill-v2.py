...
Corrected version: triggers explicit deploy after each env var change
and waits for it to complete before verifying.
"""
...
def trigger_deploy(service_id):
    """Trigger a new deploy and return its ID."""
    data = render_api("POST", f"/services/{service_id}/deploys")
    if isinstance(data, dict):
        d = data.get("deploy", data)
        return d.get("id", "")
    return ""

def get_deploy(service_id, deploy_id):
    """Get a specific deploy's status."""
    try:
        data = render_api("GET", f"/services/{service_id}/deploys/{deploy_id}")
        if isinstance(data, dict):
            d = data.get("deploy", data)
            return d.get("status", "")
    except:
...
def wait_for_deploy(service_id, deploy_id, timeout=300):
    print(f"  Waiting for deploy {deploy_id}...")
    start = time.time()
    while time.time() - start < timeout:
        status = get_deploy(service_id, deploy_id)
        if status in ("live", "deactivated", "error", "canceled"):
            elapsed = time.time() - start
            print(f"  Deploy {deploy_id}: {status} ({elapsed:.0f}s)")
            return status
        time.sleep(5)
...
def mint_service_token(service_name, audience, scopes, method, path, secret, kid, ttl=60):
    now = int(time.time())
...
def verify_token(secret, kid, label=""):
    """Mint a token with the given secret/kid and call GET /audit/events?limit=1."""
    ...
    try:
        ...
        print(f"  VERIFY {label}: FAIL (HTTP {e.code} - {body[:80]})")
        return False
...
def main():
    print("=== M3-07 Key Rotation Drill (v2) ===\n")
    
    # Step 0: Read current env vars and restore original state
    ...
    print(f"  Current key: {active_kid}")
    print(f"  All keys: {list(keys_registry.keys())}")
    
    # Use the current active key as the "old" key
    old_kid = active_kid
    old_secret = keys_registry[old_kid]["secret"]
    
    # If there are multiple keys (from a partial drill), restore to single-key state
    if len(keys_registry) > 1 or old_kid != "stg-audit-k2":
        print(f"  Restoring to single-key state with {old_kid}...")
        single_key_raw = f"{old_kid}:active:{old_secret}"
        update_env_vars(AUDIT_SERVICE_ID, {
            "WASLA_SERVICE_AUTH_KEYS": single_key_raw,
            "WASLA_SERVICE_AUTH_ACTIVE_KID": old_kid,
        })
        deploy_id = trigger_deploy(AUDIT_SERVICE_ID)
        wait_for_deploy(AUDIT_SERVICE_ID, deploy_id)
        print(f"  Restored to single-key state.")
    
    # Generate new key
    new_kid = "stg-audit-k3"
    new_secret = generate_key_secret()
    
    # Step 1: Add new key as verify_only, keep old active
    print(f"\n[1/5] Adding new key {new_kid} as verify_only (old {old_kid} stays active)...")
    ...
    deploy_id = trigger_deploy(AUDIT_SERVICE_ID)
    status = wait_for_deploy(AUDIT_SERVICE_ID, deploy_id)
    
    # Verify: old key still works (active)
    verify_token(old_secret, old_kid, "step1-old-active")
    # Verify: new key can verify tokens (verify_only means it accepts tokens signed with it)
    verify_token(new_secret, new_kid, "step1-new-verify_only")
    ...
    # Verify: new key works (active)
    verify_token(new_secret, new_kid, "step2-new-active")
    # Verify: old key still verifies (verify_only)
    verify_token(old_secret, old_kid, "step2-old-verify_only")
    ...
    # Verify: new key works (active)
    verify_token(new_secret, new_kid, "step3-new-only")
    # Verify: old key is now rejected (not in registry → unknown_kid)
    verify_token(old_secret, old_kid, "step3-old-rejected")
    
    # Step 4: Functional verification — POST audit event with new key
    print(f"\n[4/5] Functional verification — POST audit event with rotated key...")
    ...
    results = {
        ...
        "three_states_demonstrated": True,
        "db_edits_required": False,
        "env_var_changes": True,
        "service": "wasla-audit",
        "final_state": f"{new_kid}:active (single key)",
    }
