...
if [[ -n "$OLD" && -n "$NEW" ]]; then
  run_step "بوّابةُ الحوكمة (8 فحوصٍ)" bash scripts/checks/verify-governance.sh "$OLD" "$NEW"
else
  run_step "بوّابةُ الحوكمة (8 فحوصٍ)" bash scripts/checks/verify-governance.sh
fi
