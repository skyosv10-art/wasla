...
export const DELIVERY_SCOPES = {
  ...
  /**
   * الصلاحيّةُ الثالثةَ عشرةَ (المراجعةُ 24/N · M5-13 · ADR-026 §4.27) —
   * **ثالثةٌ في عائلةِ `relay-dead-letters` لا توسيعٌ لإحداهما**.
   *
   * ولا تُضَمُّ إلى `:read` لأنَّها **تكتُبُ في دفترِ مسؤوليّةٍ**: صفٌّ
   * يحمِلُ «أَقَرَّ بهِ فلانٌ لأجلِ كذا» يُقرَأُ في تحقيقٍ. ولا تُضَمُّ إلى
   * `:requeue` لأنَّ الفعلَينِ **متناقضانِ في المعنى**: الإعادةُ تقولُ
   * «يُعالَجُ من جديدٍ» والإقرارُ يقولُ «يُترَكُ بقصدٍ»؛ ورمزٌ يملِكُ الاثنينِ
   * بنطاقٍ واحدٍ يُجعَلُ مراجِعاً لا يستطيعُ أن يقولَ «مَن لهُ أن يُسكِتَ
   * تنبيهاً» — وهوَ أولُ ما يُسأَلُ عنهُ بعدَ حادثةٍ فُقِدَ فيها حدثٌ.
   */
  relayDeadLettersAcknowledge: "delivery:ops:relay-dead-letters:acknowledge",
} as const;
