 ...
 *  ALWAYS AS IDENTITY) إلى جدولِ `dispatch_outbox`. وهذا الاختبارُ يُثبِتُ أنَّ الترقيةَ
...
describe.skipIf(!PG_ENABLED)("برهانُ الترقيةِ على قاعدةٍ مأهولةٍ (M0-34 · ADR-037)", () => {
  ...
  beforeAll(async () => {
    ...
    await db.query(
      ...
      [
        ...
        "dispatch_job",
        SEEDED_AGGREGATE_ID,
        JSON.stringify({ type: "dispatch.job_created" }),
      ],
    );
  });
  ...
  it("الصفُّ المزروعُ موجودٌ قبلَ الترقيةِ — وإلّا فالبرهانُ على فراغٍ", async () => {
    const result = await db.query(`SELECT count(*)::int AS n FROM dispatch_outbox`);
  ...
  it("الصفُّ نجا بقيمِهِ وعمودُ `sequence_number` مُعبَّأٌ", async () => {
    ...
    expect(rows[0].event_type).toBe("dispatch.job_created");
  ...
  it("التراجعُ لا يُبيدُ البياناتِ — والعمودُ يُحذَفُ", async () => {
    ...
    const { rows } = await db.query(`SELECT event_id::text AS event_id FROM dispatch_outbox`);
    expect(rows).toHaveLength(1);
    expect(rows[0].event_id).toBe(SEEDED_EVENT_ID);
    ...
    const cols = await db.query(
      `SELECT column_name FROM information_schema.columns
       WHERE table_name = 'dispatch_outbox' AND column_name = 'sequence_number'`,
    );
