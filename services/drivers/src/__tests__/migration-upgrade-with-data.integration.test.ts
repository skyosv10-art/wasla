...
describe.skipIf(!PG_ENABLED)("برهانُ الترقيةِ على قاعدةٍ مأهولةٍ (M0-34 · ADR-037 §Scope)", () => {
  ...
  beforeAll(async () => {
    ...
    await db.query(
      `INSERT INTO driver_outbox (event_id, event_type, event_version, aggregate_type, aggregate_id, payload, occurred_at)
       VALUES ($1, $2, 'v1', 'driver', $3, $4::jsonb, NOW())`,
      [
        SEEDED_EVENT_ID,
        "driver.registered",
        SEEDED_AGGREGATE_ID,
        JSON.stringify({ type: "driver.registered" }),
      ],
    );
  });
  ...
  it("الصفُّ المزروعُ موجودٌ قبلَ الترقيةِ — وإلّا فالبرهانُ على فراغٍ", async () => {
    const result = await db.query(`SELECT count(*)::int AS n FROM driver_outbox`);
  ...
  it("الصفُّ نجا بقيمِهِ والفهرسُ على الـPK", async () => {
    const { rows } = await db.query(
      `SELECT event_id::text AS event_id, event_type FROM driver_outbox`,
    );
    expect(rows).toHaveLength(1);
    expect(rows[0].event_id).toBe(SEEDED_EVENT_ID);
    expect(rows[0].event_type).toBe("driver.registered");
  ...
  it("التراجعُ لا يُبيدُ البياناتِ — والفهرسُ يعودُ", async () => {
    ...
    const { rows } = await db.query(`SELECT event_id::text AS event_id FROM driver_outbox`);
    expect(rows).toHaveLength(1);
    expect(rows[0].event_id).toBe(SEEDED_EVENT_ID);
