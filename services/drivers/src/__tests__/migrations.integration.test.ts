 ...
 *     المتتابعاتُ. انحرافُ أيٍّ منها = انحدارٌ صامتٌ بين التمثيلاتِ الثلاثةِِ
 ...
 *     على سياسةِ الأهليّةِ للإصدار 1 (`saudi-launch-v1`) مجمَّدةً
 *     (`is_frozen = TRUE`) وبقيمِ الأعمدةِ الافتراضيّةِ كاملةً (الوثائقُ
 *     المطلوبةُ · المركبةُ الأولى · منطقةُ الخدمةِ · مهلةُ السماحِ صفراً) —
 *     فالترحيلُ بلا seed ليسَ عقداً: النسخةُ 1 هي القاعدةُ التي حُكمَ بها
 *     على أوّلِ سائقٍ، وقرارُ أهليّةٍ لا يُعادُ إنتاجُهُ إن غابَ صفُّها.
 *
 * يعملُ في وظيفةِ `drivers · db-integration` القائمةِ (لا وظيفةَ جديدةَ —
...
interface PolicyV1Row {
  version: number;
  label: string;
  required_documents_ride: string[];
  required_documents_delivery: string[];
  require_primary_vehicle: boolean;
  require_service_zone: boolean;
  document_grace_days: number;
  is_frozen: boolean;
}

/**
 * قراءةُ النسخةِ 1 من سياساتِ الأهليّةِ. لا حقولٌ زمنيّةٌ هنا (لا `frozen_at`
 * في هذا الجدولِ أصلاً — التجميدُ عمودٌ منطقيٌّ `is_frozen` وحدَه)، فتُقارَنُ
 * كلُّ الأعمدةِ الجوهريّةِ حرفاً بما فيها القيمُ الافتراضيّةُ التي تركَها
 * seed العقدِ لعمودَيِ الوثائقِ المطلوبةِ.
 */
async function readPolicyV1(client: pg.Client): Promise<PolicyV1Row | undefined> {
  const result = await client.query<PolicyV1Row>(`
    SELECT version, label,
           required_documents_ride, required_documents_delivery,
           require_primary_vehicle, require_service_zone,
           document_grace_days, is_frozen
      FROM driver_eligibility_policies
     WHERE version = 1`);
...
describe.skipIf(!PG_ENABLED)("الدورةُ الكاملةُ للترحيلاتِ المولَّدةِ (M0-23)", () => {
  ...
  it("seed النسخةِ 1: سياسةُ الأهليّةِ v1 موجودةٌ ومجمَّدةٌ في القاعدتَين", async () => {
    ...
    // القيمُ المستقرّةُ فقط، وما تركَهُ seed للقيمِ الافتراضيّةِ يُقاسُ هنا
    // صريحاً (الوثائقُ المطلوبةُ · المركبةُ الأولى · منطقةُ الخدمةِ · مهلةُ
    // السماحِ) حتى يُثبتَ صفُّ v1 كاملاً لا التسميةَ والتجميدَ وحدَهما.
    expect(fromContract).toMatchObject({
      version: 1,
      label: "saudi-launch-v1",
      required_documents_ride: ["national_id", "driving_license", "vehicle_registration"],
      required_documents_delivery: ["national_id", "driving_license", "vehicle_registration"],
      require_primary_vehicle: true,
      require_service_zone: true,
      document_grace_days: 0,
      is_frozen: true,
    });
