/**
 * أدواتٌ مشتركةٌ لاختباراتِ **حدِّ السائقين** بعدَ فرضِ هويّةِ الخدمةِ
 * (`M1-04` · الموجةُ العاشرةُ · `CLM-0198`).
 *
 ...
 * **تعطيلِ الفرضِ** بدلاً من تحديثِهِ. وهذا هوَ النمطُ نفسُهُ في حدودِ العميلِ
 * والسوقِ والطلباتِ والتوصيلِ والمطابقةِ.
 *
...
import { createDriverApp, type CreateDriverAppOptions } from "../http/app.js";
import { DRIVER_SCOPES, DRIVERS_SERVICE_AUDIENCE } from "../http/service-identity.js";

/** سرٌّ اختباريٌّ بطولٍ مقبولٍ؛ لا صلةَ لهُ بأيِّ سرٍّ تشغيليٍّ. */
export const TEST_SERVICE_SECRET = "drivers-test-secret-0123456789abcdef";
export const TEST_ACTIVE_KID = "test-active";
/** سرٌّ مزوَّرٌ: يُثبتُ أنَّ التوقيعَ يُفحَصُ فعلاً، لا أنَّ الترويسةَ موجودةٌ. */
export const TEST_FORGED_SECRET = "drivers-forged-secret-0123456789abcdef";

/** كلُّ صلاحيّاتِ هذا الحدِّ: اختبارُ العقدِ يُثبتُ العقدَ لا نقصَ الصلاحيّةِ. */
export const ALL_DRIVER_SCOPES: readonly string[] = Object.values(DRIVER_SCOPES);
...
export function attachSigningInject(
  ...
  target.inject = (options: InjectOptions) => {
    ...
    // `POST /drivers` يحملُ المُنتَفِعَ في الجسمِ لا في المسارِ؛ فاستخراجُهُ من
    // الجسمِ قبلَ التوقيعِ يُتيحُ للسندِ أن يوقِّعَ نداءاتِ التسجيلِ تلقائيّاً.
    let onBehalfOfPublicId: string | undefined;
    if (method.toUpperCase() === "POST" && url.split("?")[0] === "/drivers") {
      const body = options.payload as Record<string, unknown> | undefined;
      const wpid = body?.wasla_public_id;
...
export interface DriverAppHarness {
  app: ReturnType<typeof createDriverApp>;
  keys: ServiceAuthKeyRegistry;
...
/**
 * يبني تطبيقَ السائقينَ **مفروضاً** ويلفُّ `inject` بالتوقيعِ.
 */
export function buildSignedDriverApp(
  options: Omit<CreateDriverAppOptions, "serviceIdentity">,
): DriverAppHarness {
  const keys = createTestKeyRegistry();
  const replayGuard = new InMemoryServiceTokenReplayGuard();
  const app = createDriverApp({ ...options, serviceIdentity: { keys, replayGuard } });
