/**
 * اختبارُ الدورةِ الكاملةِ للترحيلاتِ المولَّدةِ (M0-23 · ADR-024 · الموجةُ 1).
 *
 ...
 *     أبعادٍ: الجداولُ · الأعمدةُ · القيودُ · الفهارسُ · المُطلِقاتُ · الدوالُّ ·
 ...
 *
 * يعملُ في وظيفةِ `customer · db-integration` القائمةِ (لا وظيفةَ جديدةَ —
...
async function applySqlFile(client: pg.Client, path: string): Promise<void> {
  ...
  const chunks = raw
    ...
    .map((s) => s.trim())
    .filter((s) => {
      // فقرةٌ تعليقاتٍ خالصةٍ لا عباراتَ فيها تُهمََل؛ وأمّا عباراتٌ تسبقُها
      // تعليقاتٌ (رأسُ الملفِّ المولَّدِ · قسمُ الإلحاقِ المُراجَعِ) فتُطبَّقُ كما هي —
      // pg يقبلُ التعليقاتَ في رأسِ الاستعلامِ.
