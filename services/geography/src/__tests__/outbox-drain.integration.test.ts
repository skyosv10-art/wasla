/**
 * اختبارُ تكاملِ تصريفِ صندوقِ صادرِ الجغرافيا (ADR-042 · موجة 2).
 *
 * يُثبتُ نمطَ ADR-042 على PostgreSQL حقيقيّة لخدمة الجغرافيا.
 * الاختلافُ عن موجة 1: `geo_outbox` بلا `aggregate_type` — المحوّلُ
 * يُرجعُ `"geography"` كقيمةٍ ثابتة.
 *
 * Local run:
 *   DATABASE_URL=postgres://wasla:wasla@127.0.0.1:5432/wasla_geography_test \
...
import { GeographyOutboxDrainStore } from "../outbox/geo-outbox-store.js";
import {
...
describe.skipIf(!ENABLED)("GeographyOutboxDrainStore integration", () => {
  let pool: Pool;
  ...
  beforeEach(async () => {
    await pool.query(`DROP TABLE IF EXISTS geo_outbox CASCADE`);
  ...
  it("claims, delivers, and marks published", async () => {
    await pool.query(
      `INSERT INTO geo_outbox (event_id, event_type, event_version, aggregate_id, payload)
       VALUES ($1, 'geo.location_set', 'v1', 'WS-GEO-001', '{"zone":"test"}'::jsonb)`,
      [TEST_EVENT_ID],
    );
    ...
    expect(sink.delivered[0].aggregateType).toBe("geography");
    expect(sink.delivered[0].eventId).toBe(TEST_EVENT_ID);

    const rows = await pool.query(
      `SELECT published_at FROM geo_outbox WHERE event_id = $1`,
      [TEST_EVENT_ID],
    );
  ...
  it("does not stop on delivery failure", async () => {
    for (const [i, eventId] of [TEST_EVENT_ID, TEST_EVENT_ID_2, TEST_EVENT_ID_3].entries()) {
      await pool.query(
        `INSERT INTO geo_outbox (event_id, event_type, event_version, aggregate_id, payload)
         VALUES ($1, 'geo.event', 'v1', $2, '{}'::jsonb)`,
        [eventId, `WS-GEO-${i + 1}`],
      );
    }

    const failId = String(
      (await pool.query(`SELECT id FROM geo_outbox WHERE event_id = $1`, [TEST_EVENT_ID_2])).rows[0].id,
    );
  ...
  it("detects already-published rows", async () => {
    await pool.query(
      `INSERT INTO geo_outbox (event_id, event_type, event_version, aggregate_id, payload, published_at)
       VALUES ($1, 'geo.event', 'v1', 'WS-GEO-001', '{}'::jsonb, now())`,
      [TEST_EVENT_ID],
    );
  ...
  it("respects SKIP LOCKED — two drains do not overlap", async () => {
    for (let i = 0; i < 5; i++) {
      await pool.query(
        `INSERT INTO geo_outbox (event_id, event_type, event_version, aggregate_id, payload)
         VALUES ($1, 'geo.event', 'v1', $2, '{}'::jsonb)`,
        [`dddddddd-dddd-4ddd-8ddd-dddddddddd0${i}`, `WS-GEO-${i + 1}`],
      );
    }
  ...
  it("persists attempts and last_error on delivery failure (G3)", async () => {
    ...
    await pool.query(
      `INSERT INTO geo_outbox (event_id, event_type, event_version, aggregate_id, payload)
       VALUES ($1, 'geo.location_set', 'v1', 'WS-GEO-001', '{}'::jsonb)`,
      [TEST_EVENT_ID],
    );

    const rowId = String(
      (await pool.query(`SELECT id FROM geo_outbox WHERE event_id = $1`, [TEST_EVENT_ID]))
        .rows[0].id,
    );
    ...
    const afterFailure = await pool.query(
      `SELECT attempts, last_error, published_at FROM geo_outbox WHERE id = $1`,
      [rowId],
    );
    ...
    await pool.query(`UPDATE geo_outbox SET published_at = NULL WHERE id = $1`, [rowId]);
