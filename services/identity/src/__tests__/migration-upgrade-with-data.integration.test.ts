 ...
 *
 *  ترحيلُ `0001_outbox_index_pk` يُغيِّرُ فهرسَ `ix_identity_outbox_unpublished`
...
const SEEDED_AGGREGATE_ID = "00000000-0000-4000-8000-000000000001";
...
describe.skipIf(!PG_ENABLED)("برهانُ الترقيةِ على قاعدةٍ مأهولةٍ (M0-34 · ADR-037 §Scope)", () => {
  ...
  beforeAll(async () => {
    ...
    await db.query(
      `INSERT INTO identity_outbox (event_id, event_type, event_version, aggregate_id, payload, occurred_at)
       VALUES ($1, $2, 'v1', $3::uuid, $4::jsonb, NOW())`,
      [
        SEEDED_EVENT_ID,
        "identity.resolved",
        SEEDED_AGGREGATE_ID,
        JSON.stringify({ type: "identity.resolved" }),
      ],
    );
  });
  ...
  it("الصفُّ المزروعُ موجودٌ قبلَ الترقيةِ — وإلّا فالبرهانُ على فراغٍ", async () => {
    const result = await db.query(`SELECT count(*)::int AS n FROM identity_outbox`);
  ...
  it("الصفُّ نجا بقيمِهِ والفهرسُ على الـPK", async () => {
    const { rows } = await db.query(
      `SELECT event_id::text AS event_id, event_type FROM identity_outbox`,
    );
    expect(rows).toHaveLength(1);
    expect(rows[0].event_id).toBe(SEEDED_EVENT_ID);
    expect(rows[0].event_type).toBe("identity.resolved");

    // Verify the index now uses the PK column
    const idx = await db.query(
      `SELECT indexname, indexdef FROM pg_indexes
       WHERE tablename = 'identity_outbox' AND indexname = 'ix_identity_outbox_unpublished'`,
    );
  ...
  it("التراجعُ لا يُبيدُ البياناتِ — والفهرسُ يعودُ", async () => {
    ...
    const { rows } = await db.query(`SELECT event_id::text AS event_id FROM identity_outbox`);
    expect(rows).toHaveLength(1);
    expect(rows[0].event_id).toBe(SEEDED_EVENT_ID);
