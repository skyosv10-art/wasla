 ...
 *  ALWAYS AS IDENTITY) إلى جدولِ `marketplace_outbox`. وهذا الاختبارُ يُثبِتُ أنَّ الترقيةَ
...
describe.skipIf(!PG_ENABLED)("برهانُ الترقيةِ على قاعدةٍ مأهولةٍ (M0-34 · ADR-037)", () => {
  ...
  beforeAll(async () => {
    ...
    await db.query(
      `INSERT INTO marketplace_outbox
         (outbox_id, event_type, event_version, aggregate_type, aggregate_id,
          payload, occurred_at)
       VALUES ($1, $2, 'v1', $3, $4, $5::jsonb, NOW())`,
      [
        SEEDED_EVENT_ID,
        "marketplace.product_created",
        "product",
        SEEDED_AGGREGATE_ID,
        JSON.stringify({ type: "marketplace.product_created" }),
      ],
    );
  });
  ...
  it("الصفُّ المزروعُ موجودٌ قبلَ الترقيةِ — وإلّا فالبرهانُ على فراغٍ", async () => {
    const result = await db.query(`SELECT count(*)::int AS n FROM marketplace_outbox`);
  ...
  it("الصفُّ نجا بقيمِهِ وعمودُ `sequence_number` مُعبَّأٌ", async () => {
    const { rows } = await db.query(
      `SELECT outbox_id::text AS outbox_id, event_type, aggregate_type, aggregate_id,
              sequence_number
       FROM marketplace_outbox`,
    );
    expect(rows).toHaveLength(1);
    expect(rows[0].outbox_id).toBe(SEEDED_EVENT_ID);
    expect(rows[0].event_type).toBe("marketplace.product_created");
  ...
  it("التراجعُ لا يُبيدُ البياناتِ — والعمودُ يُحذَفُ", async () => {
    ...
    const { rows } = await db.query(`SELECT outbox_id::text AS outbox_id FROM marketplace_outbox`);
    expect(rows).toHaveLength(1);
    expect(rows[0].outbox_id).toBe(SEEDED_EVENT_ID);
    ...
    const cols = await db.query(
      `SELECT column_name FROM information_schema.columns
       WHERE table_name = 'marketplace_outbox' AND column_name = 'sequence_number'`,
    );
