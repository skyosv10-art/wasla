/**
 * اختبارُ الدورةِ الكاملةِ للترحيلاتِ المولَّدةِ (M0-23 · ADR-024 · الموجةُ 3 · الخاتمة).
 *
 ...
 *
 * **لا بذورَ في عقدِ السوقِ بقصدٍ معلَنٍ:** العقدُ لا يزرعُ صفّاً، وشجرةُ التصنيفاتِ
 * **فارغةٌ اليومَ بقرارِ مالكِ المنتَجِ** (`domain/category-seed.ts` يُوثِّقُه:
 * الآليّةُ موصولةٌ ومُختبَرة، ومحتوى الشجرةِ قرارٌ لم يُتّخذ بعدَ) — فالترحيلُ هنا
 * DDL خالصٌ يُطابقُ العقدَ حرفاً، ولا اختبارَ بذورٍ كما في الاشتراكاتِ التي
 * يجمّدُ `PLAN_CATALOG` أرقامَها. وحينَ يُعلَنُ محتوى الشجرةِ غداً، يُلحَقُ بقسمٍ
 * مُراجَعٍ في الترحيلِ ويُقاسُ كما في الاشتراكاتِ — لا يُدسُّ صامتاً.
 *
 * ومسارُ التشغيلِ (`migrate.ts`) يبقى يُنفّذُ العقدَ **حرفاً** لا الترحيلاتِ
 * (ADR-024 §2.3): الترحيلاتُ المولَّدةُ بديلُ التطبيقِ اليدويِّ للعقدِ في البيئاتِ
 * الجديدةِ، لا مُشغّلَ الخدمةِ اليوميَّ. وهذا الاختبارُ هو ما يمنعُ الاثنينِ من
 * الافتراقِ.
 *
 * يعملُ في وظيفةِ `marketplace · db-integration` القائمةِ (لا وظيفةَ جديدةَ —
 ...
 *
 * Local run:
 *   DATABASE_URL=postgres://wasla:wasla@127.0.0.1:5432/wasla_marketplace_test \
 *     pnpm --filter @wasla/marketplace-service test:integration
 */
...
async function applySqlFile(client: pg.Client, path: string): Promise<void> {
  ...
  const chunks = raw
    ...
    .filter((s) => {
      ...
      // تعليقاتٌ (رأسُ الملفِّ المولَّدِ) فتُطبَّقُ كما هي — pg يقبلُ التعليقاتِ في
      // رأسِ الاستعلام.
...
describe.skipIf(!PG_ENABLED)("الدورةُ الكاملةُ للترحيلاتِ المولَّدةِ (M0-23)", () => {
  ...
  beforeAll(async () => {
    ...
    // القاعدةُ الأولى: العقدُ (المصدرُ القانونيُّ المكتوبُ — بلا بذورٍ بقصدٍ معلَنٍ).
