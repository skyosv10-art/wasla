 ...
 *  2. **لا بذرَ في `applyMarketplaceSchema`**: البذرُ يعيش في `db/migrate-cli.ts` بعدَ المخطّطِ،
 *     لأنّ المُهاجرةَ تُنفّذ `contracts/schema.sql` حرفيّاً ولأنّ `resetData` يُفرِّغ الجداولَ
 *     بين الاختبارات — فبذرةٌ في المخطّطِ كانت ستُمحى بعد أوّلِ اختبارٍ ثمّ تُفتقَد في الثاني.
 *     ولذلك `TRUNCATE` يشمل الجداولَ العشرةَ كلَّها بلا استثناء، وكلُّ اختبارٍ يُنشئ تصنيفَه
 *     بنفسِه (`seedLeafCategory`). والبذرةُ المُعلَنةُ في المجالِ فارغةٌ بقرارٍ، وآليّةُ البذرِ
 *     مفحوصةٌ في `categories-seed.integration.test.ts` بشجرةٍ محلّيّةٍ لا بالمُعلَنة.
