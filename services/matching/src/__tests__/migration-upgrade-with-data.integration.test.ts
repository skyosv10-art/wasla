 ...
 *  ALWAYS AS IDENTITY) إلى جدولِ `matching_outbox`. وهذا الاختبارُ يُثبِتُ أنَّ الترقيةَ
...
describe.skipIf(!PG_ENABLED)("برهانُ الترقيةِ على قاعدةٍ مأهولةٍ (M0-34 · ADR-037)", () => {
  ...
  beforeAll(async () => {
    ...
    await db.query(
      `INSERT INTO matching_outbox
         (event_id, event_type, event_version, aggregate_type, aggregate_id,
          payload, occurred_at, trace_id)
       VALUES ($1, $2, 'v1', $3, $4, $5::jsonb, NOW(), 'trace-upgrade-proof')`,
      [
        SEEDED_EVENT_ID,
        "matching.decision_recorded",
        "matching_decision",
        SEEDED_AGGREGATE_ID,
        JSON.stringify({ type: "matching.decision_recorded" }),
      ],
    );
  });
  ...
  it("الصفُّ المزروعُ موجودٌ قبلَ الترقيةِ — وإلّا فالبرهانُ على فراغٍ", async () => {
    const result = await db.query(`SELECT count(*)::int AS n FROM matching_outbox`);
  ...
  it("الصفُّ نجا بقيمِهِ وعمودُ `sequence_number` مُعبَّأٌ", async () => {
    ...
    expect(rows[0].event_type).toBe("matching.decision_recorded");
  ...
  it("التراجعُ لا يُبيدُ البياناتِ — والعمودُ يُحذَفُ", async () => {
    ...
    const { rows } = await db.query(`SELECT event_id::text AS event_id FROM matching_outbox`);
    expect(rows).toHaveLength(1);
    expect(rows[0].event_id).toBe(SEEDED_EVENT_ID);
    ...
    const cols = await db.query(
      `SELECT column_name FROM information_schema.columns
       WHERE table_name = 'matching_outbox' AND column_name = 'sequence_number'`,
    );
