 ...
 *     المتتابعاتُ. انحرافُ أيٍّ منها = انحدارٌ صامتٌ بين التمثيلاتِ الثلاثةِ
 ...
 *     على ruleset الإصدار 1 (`phase07-mvp-zone-and-fairness`) مجمّداً
 *     (`is_frozen = TRUE` و`frozen_at IS NOT NULL`) — فالترحيلُ بلا seed ليس
 *     عقداً: النسخةُ 1 هي الانحرافُ المُعلَنُ عن §30.2 (ADR-011 القرار 6).
 *
 * يعملُ في وظيفةِ `matching · db-integration` القائمةِ (لا وظيفةَ جديدةَ —
...
interface RulesetV1Row {
  version: number;
  label: string;
  w_eta: number;
  w_distance: number;
  w_zone_proximity: number;
  w_completion: number;
  w_rating: number;
  w_acceptance: number;
  w_fairness: number;
  ...
  is_frozen: boolean;
  frozen_at_is_not_null: boolean;
}

/** قراءةُ النسخةِ 1 من rulesets بتمطيطِ زمنيٍّ مستقرٍّ (is_not_null لا قيمةً). */
async function readRulesetV1(client: pg.Client): Promise<RulesetV1Row | undefined> {
  const result = await client.query<RulesetV1Row>(`
    SELECT version, label,
           w_eta, w_distance, w_zone_proximity, w_completion,
           w_rating, w_acceptance, w_fairness,
           candidacy_freshness_seconds,
           max_candidates,
           fairness_horizon_seconds,
           is_frozen,
           (frozen_at IS NOT NULL) AS frozen_at_is_not_null
      FROM matching_rulesets
     WHERE version = 1`);
...
describe.skipIf(!PG_ENABLED)("الدورةُ الكاملةُ للترحيلاتِ المولَّدةِ (M0-23)", () => {
  ...
  it("seed النسخةِ 1: ruleset v1 موجودٌ ومجمّدٌ في القاعدتَين", async () => {
    const fromContract = await readRulesetV1(contractDb);
    const fromMigrations = await readRulesetV1(migrationDb);
    ...
    // القيمُ المستقرّةُ لا الزمنيّةُ: frozen_at_is_not_null لا frozen_at كقيمةٍ.
    // تُفحَصُ الحقولُ الافتراضيّةُ كذلك (candidacy_freshness_seconds · max_candidates ·
    // fairness_horizon_seconds) حتى يُثبتَ صفُّ v1 كاملاً لا الأوزانَ وحدَها.
    expect(fromContract).toMatchObject({
      ...
      w_acceptance: 20,
      w_fairness: 20,
      candidacy_freshness_seconds: 120,
      max_candidates: 20,
      fairness_horizon_seconds: 3600,
      is_frozen: true,
      frozen_at_is_not_null: true,
    });
  ...
  it("إعادةُ التطبيقِ بعدَ الترجعِ: الدورةُ تعملُ مرّةً ثانيةً", async () => {
    ...
    const restoredSeed = await readRulesetV1(migrationDb);
