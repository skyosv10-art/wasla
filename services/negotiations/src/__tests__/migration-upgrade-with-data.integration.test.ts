 ...
 *  ALWAYS AS IDENTITY) إلى جدولِ `negotiation_outbox`. وهذا الاختبارُ يُثبِتُ أنَّ الترقيةَ
...
describe.skipIf(!PG_ENABLED)("برهانُ الترقيةِ على قاعدةٍ مأهولةٍ (M0-34 · ADR-037)", () => {
  ...
  beforeAll(async () => {
    ...
    await db.query(
      `INSERT INTO negotiation_outbox
  ...
  it("الصفُّ المزروعُ موجودٌ قبلَ الترقيةِ — وإلّا فالبرهانُ على فراغٍ", async () => {
    const result = await db.query(`SELECT count(*)::int AS n FROM negotiation_outbox`);
  ...
  it("التراجعُ لا يُبيدُ البياناتِ — والعمودُ يُحذَفُ", async () => {
    ...
    const { rows } = await db.query(`SELECT id::text AS id FROM negotiation_outbox`);
    expect(rows).toHaveLength(1);
    expect(rows[0].id).toBe(SEEDED_EVENT_ID);
    ...
    const cols = await db.query(
      `SELECT column_name FROM information_schema.columns
       WHERE table_name = 'negotiation_outbox' AND column_name = 'sequence_number'`,
    );
