...
describe.skipIf(!PG_ENABLED)("OrderOutboxDrainStore integration", () => {
  ...
  it("persists attempts and last_error on delivery failure (G3)", async () => {
    ...
    await fixture.pool.query(
      `INSERT INTO order_outbox (event_id, event_type, event_version, aggregate_type, aggregate_id, payload)
       VALUES ($1, 'order.created', 'v1', 'order', 'ORD-001', '{}'::jsonb)`,
      [TEST_EVENT_ID],
    );

    const rowId = String(
      (await fixture.pool.query(`SELECT event_id FROM order_outbox WHERE event_id = $1`, [TEST_EVENT_ID]))
        .rows[0].event_id,
    );
    ...
    const afterFailure = await fixture.pool.query(
      `SELECT attempts, last_error, published_at FROM order_outbox WHERE event_id = $1`,
      [rowId],
    );
    ...
    await fixture.pool.query(`UPDATE order_outbox SET published_at = NULL WHERE event_id = $1`, [rowId]);
