 ...
 *  ALWAYS AS IDENTITY) إلى جدولِ `reputation_outbox`. وهذا الاختبارُ يُثبِتُ أنَّ الترقيةَ
...
describe.skipIf(!PG_ENABLED)("برهانُ الترقيةِ على قاعدةٍ مأهولةٍ (M0-34 · ADR-037)", () => {
  ...
  beforeAll(async () => {
    ...
    await db.query(
      ...
      [
        ...
        "reputation_subject",
        SEEDED_AGGREGATE_ID,
        JSON.stringify({ type: "reputation.fact_recorded" }),
      ],
    );
  });
  ...
  it("الصفُّ المزروعُ موجودٌ قبلَ الترقيةِ — وإلّا فالبرهانُ على فراغٍ", async () => {
    const result = await db.query(`SELECT count(*)::int AS n FROM reputation_outbox`);
  ...
  it("الصفُّ نجا بقيمِهِ وعمودُ `sequence_number` مُعبَّأٌ", async () => {
    const { rows } = await db.query(
      `SELECT id::text AS id, event_type, aggregate_type, aggregate_id,
              sequence_number
       FROM reputation_outbox`,
    );
    expect(rows).toHaveLength(1);
    expect(rows[0].id).toBe(SEEDED_EVENT_ID);
    expect(rows[0].event_type).toBe("reputation.fact_recorded");
  ...
  it("التراجعُ لا يُبيدُ البياناتِ — والعمودُ يُحذَفُ", async () => {
    ...
    const { rows } = await db.query(`SELECT id::text AS id FROM reputation_outbox`);
    expect(rows).toHaveLength(1);
    expect(rows[0].id).toBe(SEEDED_EVENT_ID);
    ...
    const cols = await db.query(
      `SELECT column_name FROM information_schema.columns
       WHERE table_name = 'reputation_outbox' AND column_name = 'sequence_number'`,
    );
