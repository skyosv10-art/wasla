...
describe("لا شبكةَ ولا قاعدةَ بيانات ولا نظامَ ملفات", () => {
  ...
  it("التبعيّاتُ المُعلَنةُ هي العقودُ ومُشغّلُ القاعدةِ وحدها — لا HTTP ولا سواه", () => {
    ...
    // و`@wasla/config` أُضيفت بقرارٍ موثَّقٍ لا بإغفالٍ (`M2-04` · `CLM-0194` ·
    // [ADR-032]): قراءةُ منفذِ الخدمةِ كانت `Number(process.env.PORT)` فتقبلُ «٣»
    // فتصيرُ NaN بصمتٍ، فصارت تُقرأُ بقارئٍ صارمٍ من مصدرِ الحقيقةِ الواحدِ. والحزمةُ
    // بلا شبكةٍ ولا قرصٍ ولا قاعدةٍ — تقرأُ `process.env` وترمي خطأً مُسمّىً — فلا
    // تُخالِفُ نقاءَ هذا الملفِّ. والقائمةُ تبقى مكتوبةً صريحةً بأسمائِها كي تسقطَ
    // و`@wasla/service-auth` و`@wasla/auth-sdk` أُضيفتا بقرارٍ موثَّقٍ (`M1-04` ·
    // `CLM-0199` · الموجةُ الحاديةَ عشرةَ): فرضُ هويّةِ الخدمةِ على حدِّ السمعةِ،
    // كلاهما حزمٌ داخليةٌ بلا شبكةٍ ولا قرصٍ ولا قاعدةٍ — فلا تُخالِفانِ نقاءَ هذا
    // الملفِّ. والقائمةُ تبقى مكتوبةً صريحةً بأسمائِها كي تسقطَ
    // أوّلُ تبعيّةٍ تُضاف بلا قرارٍ موثَّقٍ.
