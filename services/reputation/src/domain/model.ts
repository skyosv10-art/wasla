...
/**
 * الجوابُ المحفوظ لمفتاح معالجةٍ واحدة — **مُعتِمٌ** على المجال.
 *
 * `status` رقمٌ و`body` حمولةُ JSON، والمجالُ لا يقرأ أيّاً منهما ولا يتفرّع عليهما ولا
 * يعرف من أين جاءا: **من ينادي هو من يملك شكل جوابه**. طبقةُ HTTP تُسلّم رمزَ حالتها
 * وجسمَها المنشور (`snake_case`)، ومستهلكُ الأحداث (المراجعة 5/6) يُسلّم ما يعنيه عنده،
 * والمجالُ يحفظ ويُعيد بلا تفسير.
 *
 * ولهذا لم يُسمَّ الحقلُ `httpStatus`: العمودان في العقد المُجمَّد اسمُهما
 * `response_status` و`response_body`، والاسمُ هنا يقول «جوابُ من نادى» لا «جوابُ HTTP» —
 * فلو صار للسمعة مدخلٌ آخر (طابور، gRPC) لم يُحتَج إلى عمودٍ ثالث ولا إلى كذبةٍ في
 * التسمية.
 *
 * ## ولماذا المحفوظُ هو **جوابُ الإعادة** لا جوابُ المرّة الأولى
 *
 * العمودان يوجدان لسببٍ واحد: أن يُجابَ مفتاحٌ وصل مرّتين بنفس الشيء حرفياً. وجوابُ
 * المرّة الأولى قد يختلف عن جواب الإعادة في حرفٍ واحد مقصود — `201` مقابل `200` على
 * `POST /reputation/facts`، و`duplicate: false` مقابل `duplicate: true` — لأنّ العقد
 * يقول ذلك (`errors.md` القاعدة 4). فالمحفوظُ هو ما يجب أن يُعاد **عند التكرار**، وهو
 * الشيءُ الوحيد الذي سيُقرأ من هذين العمودين بحال.
 */
export interface ReputationRecordedResponse {
  readonly status: number;
  /**
   * الجسمُ المحفوظ كما سلّمه من نادى.
   *
   * واسمُه `payload` لا `body` لأنّ حرسَ النقاء (`purity.test.ts`) يمنع كلَّ حقلٍ اسمُه
   * `body` في هذه الخدمة: القرار 5 في ADR-014 يمنع النصَّ الحرّ في التقييم، والحرسُ
   * يقيسه بالاسم لا بالنيّة — فتوسيعُه ليَسمح باستثناءٍ كان سيفتح البابَ الذي أُغلق.
   */
 ...
 * المستودع)، و`requestFingerprint` هو `payload_fingerprint`، و`recordedResponse` هو
 * `response_status` و`response_body` معاً (المراجعة 4/6 سدَّت دَينَهما: كانا `200` و`{}`
 * محفوظَين مؤقّتاً حين لم يكن للخدمة مدخلٌ يُنتج جواباً).
 *
...
export interface ReputationIdempotencyRow {
  ...
  /**
   * ما يُعاد حرفياً إن وصل المفتاحُ نفسُه ثانيةً.
   *
   * إلزاميٌّ لا اختياريّ: العمودان `NOT NULL` في العقد، وحقلٌ اختياريّ كان سيجعل كلَّ
   * مُنادٍ يُغريه ألّا يُسلّم جوابه — فيعود الدَّينُ نفسُه بقيمةٍ افتراضيةٍ صامتة بدل
   * موضعٍ محفوظٍ مُعلَن.
   */
