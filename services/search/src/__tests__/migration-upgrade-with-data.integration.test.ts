...
describe.skipIf(!PG_ENABLED)("برهانُ الترقيةِ على قاعدةٍ مأهولةٍ (M0-34 · ADR-037 §Scope)", () => {
  ...
  beforeAll(async () => {
    ...
    await db.query(
      ...
      [
        ...
        "search.index_built",
        SEEDED_AGGREGATE_ID,
        JSON.stringify({ type: "search.index_built" }),
      ],
    );
  });
  ...
  it("الصفُّ المزروعُ موجودٌ قبلَ الترقيةِ — وإلّا فالبرهانُ على فراغٍ", async () => {
    const result = await db.query(`SELECT count(*)::int AS n FROM search_outbox`);
  ...
  it("الصفُّ نجا بقيمِهِ والفهرسُ على الـPK", async () => {
    const { rows } = await db.query(
      `SELECT event_id::text AS event_id, event_type FROM search_outbox`,
    );
    expect(rows).toHaveLength(1);
    expect(rows[0].event_id).toBe(SEEDED_EVENT_ID);
    expect(rows[0].event_type).toBe("search.index_built");
  ...
  it("التراجعُ لا يُبيدُ البياناتِ — والفهرسُ يعودُ", async () => {
    ...
    const { rows } = await db.query(`SELECT event_id::text AS event_id FROM search_outbox`);
    expect(rows).toHaveLength(1);
    expect(rows[0].event_id).toBe(SEEDED_EVENT_ID);
