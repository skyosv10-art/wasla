...
describe.skipIf(!PG_ENABLED)("SearchOutboxDrainStore integration", () => {
  ...
  it("persists attempts and last_error on delivery failure (G3)", async () => {
    ...
    await fixture.pool.query(
      `INSERT INTO search_outbox (event_id, event_type, event_version, aggregate_id, payload)
       VALUES ($1, 'search.index_built', 'v1', 'IDX-001', '{}'::jsonb)`,
      [TEST_EVENT_ID],
    );

    const rowId = String(
      (await fixture.pool.query(`SELECT id FROM search_outbox WHERE event_id = $1`, [TEST_EVENT_ID]))
        .rows[0].id,
    );
    ...
    const afterFailure = await fixture.pool.query(
      `SELECT attempts, last_error, published_at FROM search_outbox WHERE id = $1`,
      [rowId],
    );
    ...
    await fixture.pool.query(`UPDATE search_outbox SET published_at = NULL WHERE id = $1`, [rowId]);
  ...
  it("reads trace_id from the row into OutboxRecord (G4)", async () => {
    ...
    const TRACE = "trace-search-g4-0001";
    await fixture.pool.query(
      `INSERT INTO search_outbox (event_id, event_type, event_version, aggregate_id, payload, trace_id)
       VALUES ($1, 'search.index_updated', 'v1', 'product-001', '{}'::jsonb, $2)`,
      [TEST_EVENT_ID, TRACE],
    );
  ...
  it("returns null trace_id when the column is null (G4)", async () => {
    ...
    await fixture.pool.query(
      `INSERT INTO search_outbox (event_id, event_type, event_version, aggregate_id, payload)
       VALUES ($1, 'search.index_updated', 'v1', 'product-002', '{}'::jsonb)`,
      [TEST_EVENT_ID_2],
    );
