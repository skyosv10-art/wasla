...
-- ═══════════════════════════════════════════════════════════════════════
-- إلحاقٌ مُراجَعٌ — خارجَ نطاقِ التوليدِ (ADR-024 §2.2)
--
-- drizzle-kit لا يُولِّدُ `INSERT` من `pgTable`، وبذورُ كتالوجِ الخطط جزءٌ من
-- المجالِ (`src/domain/plans.ts` · `PLAN_CATALOG`/`LAUNCH_PLAN`) لا من العقدِ
-- (`contracts/schema.sql` يُنشئُ الجداولَ ولا يزرعُ صفّاً — دَينٌ مُعلَنٌ في المراجعة
-- 1/6). هذا الإلحاقُ يُنسخُ القيمَ **حرفيّاً** من `LAUNCH_PLAN` الثابتِ المجمَّدِ
-- (`Object.freeze`)، فالترحيلُ وحدَهُ يُنتجُ قاعدةً قابلةً للتفسيرِ: كلُّ مُدّةٍ
-- ومكافأةٍ وانتقالٍ يُفسَّرُ بالخطّةِ التي حكمتْه. والمصدرُ القانونيُّ يبقى
-- `domain/plans.ts`؛ هذا الإلحاقُ ترحيلٌ لا مصدرَ ثانياً.
-- ═══════════════════════════════════════════════════════════════════════

-- النسخةُ 1: خطّةُ الإطلاق `saudi-driver-monthly` مجمَّدةً بكلِّ أرقامِها المُعلَنةِ
-- (المُشتقّةِ من `@wasla/contracts-subscription`). من يريدُ غيرَها يُصدرُ نسخةَ
-- خطّةٍ جديدةً ويُبرّرُها في ADR — لا يُعدِّلُ هذا الصفَّ.
INSERT INTO "subscription_plans" (
    "plan_code", "plan_version", "label",
...
VALUES (
    'saudi-driver-monthly', 1, 'اشتراك السائق الشهري',
    14, 30,
    7, 3,
    30, 5, 30,
    true, '2026-02-01T00:00:00+00:00'
)
ON CONFLICT ("plan_code", "plan_version") DO NOTHING;

-- استحقاقاتُ النسخةِ 1 الأربع: `accept_orders` بلا سقفٍ (-1)، و`daily_order_cap` للخطّةِ
-- المدفوعةِ (12)، و`priority_dispatch` تشغيلٌ (1)، و`zone_multi_select` ثلاثُ نطاقاتٍ (3).
INSERT INTO "subscription_plan_entitlements" (
    "plan_code", "plan_version", "entitlement_code", "limit_value"
)
VALUES
    ('saudi-driver-monthly', 1, 'accept_orders',      -1),
    ('saudi-driver-monthly', 1, 'daily_order_cap',     12),
    ('saudi-driver-monthly', 1, 'priority_dispatch',   1),
    ('saudi-driver-monthly', 1, 'zone_multi_select',   3)
ON CONFLICT ("plan_code", "plan_version", "entitlement_code") DO NOTHING;
