 ...
 *
 * عقدُ هذه الخدمة بلا بذورٍ — `contracts/schema.sql` بلا `INSERT`: كلُّ بياناتِ
 * الخطّةِ يبذرها المُهاجرةُ (`migrateSubscriptions`) برمجيّاً لا العقدُ، فلا قسمَ
 * للبذورِ هنا (بخلافِ الـreputation الذي يبذرُه العقدُ نفسُه).
 *
...
describe.skipIf(!PG_ENABLED)("الدورةُ الكاملةُ للترحيلاتِ المولَّدةِ (M0-23)", () => {
  ...
  beforeAll(async () => {
    ...
    // القاعدةُ الأولى: العقدُ (المصدرُ القانونيُّ المكتوبُ — وهو هنا بلا بذور).
